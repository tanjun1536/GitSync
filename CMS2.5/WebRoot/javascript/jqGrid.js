...
(function() {
	...
	window.jqGrid.crateJqGridButtonColumn = function(object, data) {
		...
		if (typeof object.action != 'undefined') {
			if (typeof object.extor != 'undefined') {
				but += " onclick ='" + object.action + "(" + data.id + ",&apos;"+data[object.extor]+"&apos;)'";
			} else {
				but += " onclick ='" + object.action + "(" + data.id + ")'";
			}
	...
	window.jqGrid.createJqGridSeparateButton = function(table, pager, url, datatype, headers, columns, pagesize, caption, srfunction, buttons) {
		$(table).jqGrid({
			...
			loadComplete : function() {
				if (buttons != undefined && buttons.length > 0) {
					...
					for ( var i = 0; i < ids.length; i++) {
						...
						for ( var j = 0; j < buttons.length; j++) {
							...
							var button = jqGrid.crateJqGridButtonColumn(buttonObj, data);
							var json = '({"' + gc + '":"' + button + '"})';
				...
				$(table).setGridWidth($(window).width()*0.99,true);
			}
		});
		...
		$(table).jqGrid({
			...
			loadComplete : function() {
				...
				$(table).setGridWidth($(window).width()*0.99);
			}
		});
