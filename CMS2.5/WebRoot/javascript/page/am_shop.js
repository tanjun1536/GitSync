		...
		for ( var i = 0; i < imagesGroups.length; i++) {
			...
			if (group.images) {
				for ( var j = 0; j < group.images.length; j++) {
					addFileByURLAndDesc('images', group.images[j].url, group.images[j].des,i);
				}
			}
		}
	}
		...
		for ( var i = 0; i < videosGroups.length; i++) {
			...
			if (group.videos) {
				for ( var j = 0; j < group.videos.length; j++) {
					addFileByURLAndDesc('videos', group.videos[j].url, group.videos[j].des,i);
				}
			}
		}
	}
...
function addFileByURLAndDesc(container, url, desc,selected) {
	var jqContainer = '#' + container;
	//var selected = $(jqContainer).tabs('option', 'selected');
	selected++;
	...
	var content = '<li  style="width:120px;"><img onclick="$(this).parent().remove();" title="删除图片" src="images/close.png" style="cursor:pointer;padding-left:104px;" /><img  width="120" height="80" src="'
			+ url + '" alt="picture" ondblclick="editFile(\'images\',this)"> <span>' + desc + '</span></li>';
	if (container == 'videos')
		content = '<li  style="width:120px;"><img onclick="$(this).parent().remove();" title="删除图片" src="images/close.png" style="cursor:pointer;padding-left:104px;" /><video preload="auto" width="120" height="80" src="'
				+ url + '" ondblclick="editFile(\'videos\',this)" ></video> <span>' + desc + '</span></li>';
...
function addAssist(title, container) {
	$.dialog.data('container', container);
	var d=$.dialog({
	    lock: true,
	    title : title,
	    ...
	    opacity: 0.5,       /* 透明度 */
	    content: '键：<input id="key" type="text" /><br/><br/>值：<input id="val" type="text"/>',
	    ok: function () {
	    	$(container).append('<li><label class="label"></label><span>' + $('#key',parent.document).val() + ':'+$('#val',parent.document).val()+'</span><div style="float:right" ><img src="images/close.png" onclick="$(this).parent().parent().remove();" title="删除" style="cursor:pointer" /></div></li> ');
	    	adjustAssist();
	    	return true;
	    },
	    cancel: true
...
function adjustAssist()
{
	var kv=[];
	$("#assist li").each(function(index) {
		kv.push($(this).find("span").text());
	});
	$("#basicDesc").val(kv.join("|"));
}
function save() {
	adjust();
	adjustRelated();
