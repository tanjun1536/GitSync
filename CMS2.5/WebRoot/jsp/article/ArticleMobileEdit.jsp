...
<script type="text/javascript">
	...
	$(function() {
		...
		oldIndex=${content.contentTemplate.id};
		$("#contentTemplate\\.id").change(function() {
			...
			$("#contentContainer").html(map.get($("#contentTemplate\\.id").val()));
			createShowType("#contentTemplate\\.id");
		});
		$("#section\\.id").combotree('setValue', ${content.section.id});
		$("#contentTemplate\\.id").val(${content.contentTemplate.id});
		createShowType("#contentTemplate\\.id");
		$("#contentShowType\\.id").val(${content.contentShowType.id});
		$("#contentType\\.id").val(${content.contentType.id});
		changeTemplate($("#contentTemplate\\.id"));
		if("${content.contentType.id}"=="1" || "${content.contentType.id}"=="3")
		{
	...
	function createShowType(templateObj) {
		var showtypes = $(templateObj).find('option:selected').attr('showtypes');
...
<body>
	...
	<div class="header_c">当前位置：采编管理 > 手机内容编辑</div>
	<div class="clear"></div>
	</header>
	<form id="ContentForm" method="post">
		...
		<div class="formdiv">
			<ol>
				...
				<div id="contentContainer">${HTML}</div>

			</ol>
			<div style="padding: 40px 110px; border-top: 1px dotted #CCC">
			<c:if test="${content.state.id==1 || content.state.id==5 }">
				<input type="button" value="保存" class="btn" onfocus="this.blur()"
					onclick="update()" onMouseOver="this.className='btn_over'"
					onMouseOut="this.className='btn'" style="vertical-align: top" /></c:if> <input
