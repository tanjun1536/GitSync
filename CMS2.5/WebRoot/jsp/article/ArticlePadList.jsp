...
<html>
	...
	var tree;
	$(function() {
		var headers="编号,预览,编辑,删除,日志,评论,分类,状态,标题,焦点,访问,时间";
		//创建列
		...
		//创建Grid
		window.jqGrid.createJqGridSeparateButton("#gridTable", "#gridPager", "ArticlePadAction!getList.action", "json",headers, columns, 10,"文章列表", undefined,buttons);
		//选中年月日
		getStates();
	...
	{
		var url="ArticlePadAction!getList.action";
		var where="";
	...
	function query()
	{
		...
		var url="ArticlePadAction!getList.action?startDate="+startDate;
			url+="&endDate="+endDate;
			if(keyword!='')
			{
	...
	function addAtricle()
	{
		fn.openURL("<%=basePath%>ArticlePadAction!add.action");
	}
	//修改
	function update(id) {
		fn.openURL("<%=basePath%>ArticlePadAction!edit.action?id=" + id);
	}
	//删除
	function del(id) {
		if (confirm("是否删除该记录？")) {
			$.ajax({
				type : "POST",
				url : "ArticlePadAction!ajaxDelete.action",
				datatype : "json",
				data : {
					'id' : id
				},
	...
	function batchdel() {
		...
		if (confirm("确认要删除选择的文章吗？")) {
			$.ajax({
				type : "POST",
				url : "ArticlePadAction!ajaxBatchDelete.action",
				datatype : "json",
				data : {
					'ids' : ids
				},
	...
	function showcomment(id) {
		$.dialog({
		id:'preview',
		content:"url:<%=basePath%>jsp/comments/ViewArticlePadCommentList.jsp?id="+id,
		max:false,
		min:false,
		width:'700px',
		height:500,
	...
	function showlog(id){
		fn.openURL("<%=basePath%>jsp/log/StateLogList.jsp?type=<%=StateLog.Pad_Article%>&dataId="+id);
	}
</script>
	...
	<header>
		...
		<div class="sreachdiv">
			<div style="padding-bottom: 8px; margin-bottom: 8px; border-bottom: 1px dotted #CCC">
				<input type="button" onclick="addAtricle();" value="新建" class="btn" onfocus="this.blur()" onMouseOver="this.className='btn_over'" onMouseOut="this.className='btn'" style="vertical-align: top" />
				<input id="article.section.id" name="article.section.id" type="hidden" class="easyui-combotree" url="SectionAction!getSectionTreeJSON.action" style="width: 200px;">
					...
					<input type="button" onclick="javascript:batchdel();" id="doaction" name="doaction" value="批量删除" class="btn" onfocus="this.blur()" onMouseOver="this.className='btn_over'" onMouseOut="this.className='btn'" style="vertical-align: top" />
			</div>
			<ol>
				<li>
					<label class="label">
					...
					<input type="text" id="startdate" class="Wdate" onfocus="WdatePicker()" />
				</li>
				<li>
					<label class="label">
						结束时间:
					</label>
					<input type="text" id="enddate" class="Wdate" onfocus="WdatePicker()" />
				</li>
