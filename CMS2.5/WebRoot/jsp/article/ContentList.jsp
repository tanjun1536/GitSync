...
<html>
	...
	var tree;
	$(function() {
		var headers="编号,模版,预览,编辑,删除,日志,评论,分类,状态,标题,焦点,访问,时间";
		//创建列
		var columns = new Array();
		columns.push("{'n':'id','s':'int','w':10,'h':true}");
		columns.push("{'n':'contentTemplate.id','s':'int','w':10,'h':true}");
		...
		//创建GridW
		window.jqGrid.createJqGridSeparateButton("#gridTable", "#gridPager", "ContentMobileAction!getList.action", "json",headers, columns, 10,"文章列表", undefined,buttons);
...
// 		getStates();
	...
	function getStates()
	{
		$.ajax({
				type : "POST",
				url : "ContentMobileAction!getState.action",
				datatype : "json",
	...
	{
		var url="ContentMobileAction!getList.action";
		var where="";
	...
	function query()
	{
		...
		var url="ContentMobileAction!getList.action?startDate="+startDate;
			url+="&endDate="+endDate;
			if(keyword!='')
			{
	...
	function addAtricle()
	{
		fn.openURL("<%=basePath%>ContentMobileAction!add.action");
	}
	//修改
	function update(id,tid) {
		fn.openURL("<%=basePath%>ContentMobileAction!edit.action?id=" + id+"&template="+tid);
	}
	//删除
	function del(id) {
		if (confirm("是否删除该记录？")) {
			$.ajax({
				type : "POST",
				url : "ContentMobileAction!delete.action",
				datatype : "json",
				data : {
					'id' : id
				},
	...
	function batchdel() {
		...
		if (confirm("确认要删除选择的文章吗？")) {
			$.ajax({
				type : "POST",
				url : "ContentMobileAction!ajaxBatchDelete.action",
				datatype : "json",
				data : {
					'ids' : ids
				},
