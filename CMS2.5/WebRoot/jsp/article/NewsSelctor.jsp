	...
	<base href="<%=basePath%>">
		...
		<script type="text/javascript">
			...
			$(function() {
				...
				window.jqGrid.createJqGridSeparateButton("#list", "#pager", "ArticleMobileAction!getRelatedNews.action?sectionId=${param.sectionId}&ams="+ams.join(","), "json", headers, columns, 10, "文章列表", undefined, buttons);
			...
			function queryBySection() {
				var url = "ArticleMobileAction!getRelatedNews.action";
				var sectionId = $("#section").combotree("getValue");
				if (sectionId != '')
				{
					url = "ArticleMobileAction!getRelatedNews.action?sectionId=" + sectionId;
					if(ams.length>0) 
					{
						url+="&ams="+ams.join(',');
					}
