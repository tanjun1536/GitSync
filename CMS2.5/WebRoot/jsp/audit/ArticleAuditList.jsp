...
<html>
	...
	var tree;
	$(function() {
		var headers="编号,历史,审核,分类,标题,焦点,访问,时间,状态";
		//创建列
		var columns = new Array();
		columns.push("{'n':'id','s':'int','w':10,'h':true}");
		columns.push("{'n':'history', 'w':45}");
		columns.push("{'n':'audit', 'w':45}");
		//columns.push("{'n':'back', 'w':45}");
		...
		buttons.push({"extor":"contentTemplate.entityName","gc":"audit","showName":"审核","action":"next","width":"16px","height":"16px"});
		//buttons.push({"gc":"back","showName":"驳回","action":"reject","width":"16px","height":"16px"});
		//创建Grid
		window.jqGrid.createJqGridNoCheckBoxSeparateButton("#gridTable", "#gridPager", "ArticleAuditAction!getList.action", "json",headers, columns, 10,"审核列表", undefined,buttons);
		//绑定新建按钮点击事件
	...
	//审核下一步
	function next(id,entity) {
		/*if (confirm("是否通过审核？")) {
			fn.openURL('ArticleAuditAction!auditNext.action?article.id=' + id);
		}*/
		fn.openURL('<%=basePath%>ArticleAuditAction!audit.action?article.id=' + id);
	}
	//删除
	function reject(id) {
		if (confirm("是否要驳回？")) {
			fn.openURL("<%=basePath%>ArticleAuditAction!toReject.action?article.id=" + id);//'ArticleAuditAction!reject.action?article.id=' + id;
		}
	}
	function history(id){
		fn.openURL("<%=basePath%>audithistory.action?id=" + id);
	}
</script>
