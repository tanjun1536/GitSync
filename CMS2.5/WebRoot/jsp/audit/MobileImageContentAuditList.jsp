...
<html>
	...
	$(function() {
		...
		columns.push("{'n':'history', 'w':25}");
		columns.push("{'n':'audit', 'w':25}");
		//columns.push("{'n':'back', 'w':25}");
		...
		//创建Grid
		window.jqGrid.createJqGridNoCheckBoxSeparateButton("#gridTable", "#gridPager", "MobileImageContentAuditAction!getList.action?state=${param.state}", "json",headers, columns, 10,"审核列表", undefined,buttons);
		//绑定新建按钮点击事件
	...
	function next(id) {
		/**if (confirm("是否通过审核？")) {
			fn.openURL('MobileImageContentAuditAction!auditNext.action?id=' + id + "&state=${param.state}");
		}*/
		fn.openURL('<%=basePath%>MobileImageContentAuditAction!audit.action?id=' + id);
	}
	//删除
	function reject(id) {
		if (confirm("是否要驳回？")) {
			fn.openURL("<%=basePath%>MobileImageContentAuditAction!toReject.action?id=" + id + "&state=${param.state}");
		}
	}
	function history(id){
		fn.openURL("<%=basePath%>MobileImageContentAuditHistory.action?id=" + id + "&state=${param.state}");
	}
</script>
