...
<html>
	...
	$(function() {
		...
		//创建Grid
		window.jqGrid.createJqGridNoCheckBoxSeparateButton("#gridTable", "#gridPager", "PadArticleAuditAction!getList.action", "json",headers, columns, 10,"审核列表", undefined,buttons);
		//绑定新建按钮点击事件
	...
	function next(id) {
		/*if (confirm("是否通过审核？")) {
			fn.openURL('PadArticleAuditAction!auditNext.action?article.id=' + id);
		}*/
		fn.openURL('<%=basePath%>PadArticleAuditAction!audit.action?article.id=' + id);
	}
	//删除
	function reject(id) {
		if (confirm("是否要驳回？")) {
			fn.openURL("<%=basePath%>PadArticleAuditAction!toReject.action?article.id=" + id);//'ArticleAuditAction!reject.action?article.id=' + id;
		}
	}
	function history(id){
		fn.openURL("<%=basePath%>PadAudithistory.action?id=" + id);
	}
</script>
