...
<html>
	...
	$(function() {
		...
		//创建Grid
		window.jqGrid.createJqGridNoCheckBoxSeparateButton("#gridTable", "#gridPager", "PadImageContentAuditAction!getList.action?state=${param.state}", "json",headers, columns, 10,"审核列表", undefined,buttons);
		//绑定新建按钮点击事件
	...
	function next(id) {
		/*if (confirm("是否通过审核？")) {
			fn.openURL('PadImageContentAuditAction!auditNext.action?id=' + id + "&state=${param.state}");
		}*/
		fn.openURL('<%=basePath%>PadImageContentAuditAction!audit.action?id=' + id);
	}
	//删除
	function reject(id) {
		if (confirm("是否要驳回？")) {
			fn.openURL("<%=basePath%>PadImageContentAuditAction!toReject.action?id=" + id + "&state=${param.state}");
		}
	}
	function history(id){
		fn.openURL("<%=basePath%>PadImageContentAuditHistory.action?id=" + id + "&state=${param.state}");
	}
</script>
