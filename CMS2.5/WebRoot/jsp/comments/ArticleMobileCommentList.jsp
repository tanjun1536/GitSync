...
<html>
	...
	$(function() {
		...
		columns.push("{'n':'userName', 'w':120,'a':'left'}");
		columns.push("{'n':'content','a':'left','w':200}");
		columns.push("{'n':'postDate'}");
		//创建Grid
		window.jqGrid.createJqGridSeparateButton("#gridTable", "#gridPager", "ArticleMobileCommentAction!auditcomments.action?id=${param.id}", "json",headers, columns, 10,"文章评论列表", undefined,[]);
	});
	function pass(){
		...
		fn.openURL('<%=basePath%>ArticleMobileCommentAction!pass.action?ids=' + ids);
	}
	function nopass(){
		var ids = window.jqGrid.getSelected("#gridTable");
		if($.trim(ids) == ""){
		...
		fn.openURL('<%=basePath%>ArticleMobileCommentAction!nopass.action?ids=' + ids);
	}
</script>
