  ...
  <head>
	...
	<script type="text/javascript">
		$(function() {
			...
			columns.push("{'n':'edit', 'w':10}");
			columns.push("{'n':'delete', 'w':10}");
			columns.push("{'n':'fullName','s':'string'}");
				//创建操作按钮
			...
			//创建Grid
			window.jqGrid.createJqGridSeparateButton("#gridTable", "#gridPager", "DepartmentAction!getDepartmentList.action", "json", "编号,编辑,删除,部门名称", columns, 10, "部门列表", undefined,buttons);
		});
		
		function update(id){
			fn.openURL('<%=basePath%>DepartmentAction!edit.action?id=' + id)
		}
		function del(id){
			fn.openURL('<%=basePath%>DepartmentAction!delete.action?id=' + id)
		}
		...
		function batchDel(){
			...
			fn.openURL('<%=basePath%>DepartmentAction!deleteList.action?ids=' + ids);
		}
		function queryByParentId()
		{
			var departId=$("#depart\\.parent\\.id").combotree("getValue");
			if(departId=='') return;
			jqGrid.reLoad('#gridTable','DepartmentAction!getDepartmentList.action?depart.parent.id=' + departId);
		}
	</script>
  </head>
	...
	<body>
		...
		<div class="sreachdiv">
			<div style="padding-bottom: 8px; margin-bottom: 8px; border-bottom: 1px dotted #CCC">
				<input type="button" value="新建" onclick="fn.openURL('<%=basePath%>DepartmentAction!add.action')" class="btn" onfocus="this.blur()" onMouseOver="this.className='btn_over'" onMouseOut="this.className='btn'" style="vertical-align: top" />
				...
				<input type="button" onclick="queryByParentId();" value="筛选" id="post-query-submit" class="btn" onfocus="this.blur()" onMouseOver="this.className='btn_over'" onMouseOut="this.className='btn'" style="vertical-align: top" />
			</div>
		</div>
		<div>
			<table id="gridTable">
