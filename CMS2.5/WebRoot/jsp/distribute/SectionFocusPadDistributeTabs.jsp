...
<html>
	...
	$(function() {
		...
		$('#tabs').bind('tabsselect', function(event, ui) {
			...
			window.jqGrid.createJqGridSeparateButton("#gridTable", "#gridPager", "", "json",headers, columns, 10,"平板焦点待发布列表", undefined,buttons);
			//已经发布====================================================================================================
			...
			buttonsOnline.push({"gc":"offLine","showName":"下线","action":"hiddenf","width":"16px","height":"16px"});
			window.jqGrid.createJqGridSeparateButton("#gridTableOnline", "#gridPagerOnline", "", "json",headersOnline, columnsOnline, 10,"平板焦点已发布列表", undefined,buttonsOnline);
			//已经下线====================================================================================================
			...
			columnsOffline.push("{'n':'online', 'w':45}");
			columnsOffline.push("{'n':'section.name', 'w':120,'a':'left'}");
			columnsOffline.push("{'n':'title','a':'left',w:200}");
			//创建操作按钮
			var buttonsOffline=new Array();
			buttonsOffline.push({"gc":"online","showName":"上线","action":"display","width":"16px","height":"16px"});
			window.jqGrid.createJqGridSeparateButton("#gridTableOffline", "#gridPagerOffline", "", "json",headersOffline, columnsOffline, 10,"平板焦点已下线列表", undefined,buttonsOffline);
		});
	...
	function toTop(id) {
		$.ajax({
			type : "POST",
			url : "SectionFocusPadDistributeAction!ajaxToTop.action",
			datatype : "json",
			data : {
				"id" : id
			},
			success : function(msg) {
				jqGrid.reLoad("#gridTableOnline", 'SectionFocusPadDistributeAction!getSectionFocusPadHasDistribute.action?section=' + section);
			},
			error : function(msg) {
				alert("删除文章失败!");
			}
	...
	function display(id) {
		$.ajax({
			type : "POST",
			url : "<%=basePath%>SectionFocusPadDistributeAction!display.action",
				datatype : "json",
				data : {
					'id' : id
				},
				success : function(msg) {
					var url = 'SectionFocusPadDistributeAction!getSectionFocusPadPrepareDistribute.action?section=' + section;
					jqGrid.reLoad("#gridTable", url);
					 url = 'SectionFocusPadDistributeAction!getSectionFocusPadHasOffline.action?section=' + section;
		...
		function hiddenf(id){
			$.ajax({
				type : "POST",
				url : "<%=basePath%>SectionFocusPadDistributeAction!hidden.action",
				datatype : "json",
				data : {
					'id' : id
				},
				success : function(msg) {
					var url = 'SectionFocusPadDistributeAction!getSectionFocusPadHasDistribute.action?section=' + section;
		...
		function distribute(id) {
			$.ajax({
				type : "POST",
				url : "<%=basePath%>SectionFocusPadDistributeAction!distribute.action",
			datatype : "json",
			data : {
				'id' : id
			},
	...
	function refreshList(id) {
		...
		var url;
		if (tabIndex == 0) {
			url = 'SectionFocusPadDistributeAction!getSectionFocusPadPrepareDistribute.action?section=' + id;
			jqGrid.reLoad("#gridTable", url);
		} else if (tabIndex == 1) {
			url = 'SectionFocusPadDistributeAction!getSectionFocusPadHasDistribute.action?section=' + id;
			$("#gridTableOnline").setGridWidth($("#gridTable").width());
			jqGrid.reLoad("#gridTableOnline", url);
		} else {
			url = 'SectionFocusPadDistributeAction!getSectionFocusPadHasOffline.action?section=' + id;
