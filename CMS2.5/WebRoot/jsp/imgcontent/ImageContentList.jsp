  ...
  <head>
	...
	<script type="text/javascript">
		$(function() {
			...
			//创建Grid
			window.jqGrid.createJqGridSeparateButton("#gridTable", "#gridPager", "ImageContentAction!getList.action", "json", headers, columns, 10, "图片内容列表", undefined, buttons);
		});
		
		function update(id){
			fn.openURL('<%=basePath%>ImageContentAction!edit.action?id=' + id)
		}
		...
		function del(id){
			if (confirm("是否删除该记录？")) {
				$.ajax({
					type : "POST",
					url : "ImageContentAction!ajaxDelete.action",
					datatype : "json",
					data : {
						"id" : id
					},
		...
		function add()
		{
			fn.openURL('<%=basePath%>ImageContentAction!add.action');
		}
		
		
		function queryBySection()
		{
			var url="ImageContentAction!getList.action";
			var where="";
		...
		function query()
		{
			...
			var url="ImageContentAction!getList.action?startDate="+startDate;
				url+="&endDate="+endDate;
				if(keyword!='')
				{
