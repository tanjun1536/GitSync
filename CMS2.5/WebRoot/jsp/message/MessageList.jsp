...
<html>
	...
	$(function() {
		...
		buttons.push({"gc":"edit","showName":"编辑","tip":"编辑此行数据","action":"edit","width":"16px","height":"16px"});
		//创建Grid
		window.jqGrid.createJqGridSeparateButton("#gridTable", "#gridPager", "MessageAction!getList.action", "json",headers, columns, 10,"文章列表", undefined,buttons);
	});
	...
	function addMessage()
	{
		fn.openURL("<%=basePath%>jsp/message/MessageArticles.jsp");
	}
	function edit(id)
	{
		fn.openURL("<%=basePath%>jsp/message/MessageArticles.jsp?id="+id);
	}
	//删除
	function del(id) {
		if (confirm("是否删除该记录？")) {
			$.ajax({
				type : "POST",
				url : "MessageAction!ajaxDelete.action",
				datatype : "json",
				data : {
					'id' : id
				},
	...
	<body>
		...
		<div class="sreachdiv">
			<div style="padding-bottom: 8px; margin-bottom: 8px; border-bottom: 1px dotted #CCC">
				<input type="button" onclick="addMessage();" value="添加消息" class="btn" onfocus="this.blur()" onMouseOver="this.className='btn_over'" onMouseOut="this.className='btn'" style="vertical-align: top" />
			</div>
		</div>
		<div>
			<table id="gridTable">
