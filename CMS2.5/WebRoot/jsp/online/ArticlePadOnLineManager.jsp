...
<html>
	...
	$(function() {
			...
			$("#section").tree({
				...
				onClick:function(node){
					section=node.id;
					jqGrid.reLoad('#gridTable','PadPageLayoutAction!getList.action?section='+section);
	...
	function add()
	{
		...
		fn.openURL('<%=basePath%>PadPageLayoutAction!add.action?state=${param.state}&section='+section);
	}
	function edit(id)
	{
		fn.openURL('<%=basePath%>PadPageLayoutAction!edit.action?id='+id);
	}
	function del(id)
	{
		if (confirm("是否删除该记录？")) {
