...
//		buttons.push({"gc":"next","showName":"下移","tip":"下移此行数据","action":"next","width":"16px","height":"16px"});
		//创建Grid
		window.jqGrid.createJqGridNoCheckBoxSeparateButton("#gridTable", "#gridPager", "PadSectionFocusAction!getList.action", "json","编号,编辑,删除,栏目,标题,状态", columns, 10, "栏目焦点列表", undefined, buttons);
		//绑定新建按钮点击事件
	});
	...
	function add()
	{
		fn.openURL("<%=basePath%>PadSectionFocusAction!add.action");
	}
	//修改
	function update(id)
	{
		fn.openURL("<%=basePath%>PadSectionFocusAction!edit.action?focus.id=" + id);
	}
	function up(id){
		fn.openURL("<%=basePath%>PadSectionFocusAction!setPoistion.action?strategy=-1&id=" + id);
	}
	function next(id){
		fn.openURL("<%=basePath%>PadSectionFocusAction!setPoistion.action?strategy=1&id=" + id);
	}
	...
	function del(id) {
		if (confirm("是否删除该记录？")) {
			$.ajax({
				type : "POST",
				url : "PadSectionFocusAction!ajaxDelete.action",
				datatype : "json",
				data : {
					"id" : id
				},
	...
	function queryBySection()
	{
		...
		if(sectionId=='')return ;
		jqGrid.reLoad('#gridTable','PadSectionFocusAction!getList.action?focus.section.id='+sectionId);
	}
	function query()
	{
		var searchBy=$("#searchField").val();
		var keyword=$("#searchkey").val();
		var url="PadSectionFocusAction!getList.action";
			if(keyword!='')
			{
