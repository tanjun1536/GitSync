...
<script type="text/javascript">
	...
	$(function() {
		...
		var headers="日期,活跃用户数量";
		//创建列
		var columns = new Array();
		columns.push("{'n':'link'}");
		columns.push("{'n':'value'}");
		//创建Grid
		window.jqGrid.createJqGridNoCheckBoxSeparateButton("#gridTable", undefined, "StatisticsAction!getActiveUser.action", "json",headers, columns, 10,"活跃用户统计(点击类型查看详细)", undefined,undefined);
	...
	function detail(date)
	{
		...
		var headers="激活码,用户昵称,用户性别,设备类型";
		//创建列
		var columns = new Array();
		...
		var URL="StatisticsAction!getActiveUserDetail.action?date="+date;
	...
	function list()
	{
		...
		columns.push("{'n':'value'}");
		//创建Grid
		window.jqGrid.createJqGridNoCheckBoxSeparateButton("#gridTable", undefined, "StatisticsAction!getActiveUser.action", "json",headers, columns, 10,"客户端分布(点击类型查看详细)", undefined,undefined);
...
<body>
	...
	<div class="header_c">当前位置：统计分析管理 > 活跃用户统计</div>
	<div class="clear"></div>
	</header>
	<div id=tabs>
		<ul>
