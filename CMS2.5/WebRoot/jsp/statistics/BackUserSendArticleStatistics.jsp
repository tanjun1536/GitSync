...
<script type="text/javascript">
	...
	$(function() {
		...
		columns.push("{'n':'c'}");
		//创建Grid
		window.jqGrid.createJqGridNoCheckBoxSeparateButton("#gridTable", undefined, "StatisticsAction!getBackUserSendArticle.action", "json",headers, columns, 10,"后台用户发文统计", undefined,undefined);
	...
	function detail(id)
	{
		...
		var headers="激活码,用户昵称,用户性别,操作类别";
		//创建列
		var columns = new Array();
			columns.push("{'n':'activeKey'}");
		columns.push("{'n':'userName'}");
		columns.push("{'n':'sex'}");
		columns.push("{'n':'activityType'}");
		var URL="StatisticsAction!getActiveUserDetail.action?id="+id;
	...
	function list()
	{
		...
		var headers="激活码,用户昵称,用户性别,操作数量";
		//创建列
		var columns = new Array();
		columns.push("{'n':'activeKey'}");
		columns.push("{'n':'userName'}");
		columns.push("{'n':'sex'}");
		columns.push("{'n':'activityTimes'}");
		//创建Grid
		window.jqGrid.createJqGridNoCheckBoxSeparateButton("#gridTable", undefined, "StatisticsAction!getBackUserSendArticle.action", "json",headers, columns, 10,"客户端分布(点击类型查看详细)", undefined,undefined);
...
<body>
	...
	<div class="header_c">当前位置：统计分析管理 > 后台用户发文统计</div>
	<div class="clear"></div>
	</header>
	<div id=tabs>
		<ul>
