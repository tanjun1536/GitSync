<?php
/*
 * File name: /include/zip.php Author:	Horace 2009/04/15
 */
...
class PHPZip {
	...
	function createZip($dir, $zipfilename, $autoDownload = false) {
		function get_basename($filename) {
			// return ltrim(basename(' ' . $filename)) ;
		...
		if (@function_exists ( 'gzcompress' )) {
			...
			if (is_array ( $dir )) {
				foreach ( $dir as $key => $value ) {
					...
					$fd = fopen ( $value, "r" );
					$fileValue = fread ( $fd, filesize ( $value ) );
					fclose ( $fd );
					$filename = get_basename ( $value );
	...
	function filezip() {
		...
		return $data . $ctrldir . $this->eof_ctrl_dir . pack ( 'v', sizeof ( $this->ctrl_dir ) ) . 		// total
		                                                                                           // #
		                                                                                           // of
		                                                                                           // entries
		                                                                                           // "on
