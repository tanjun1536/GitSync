...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="topic2">
					<a href="__URL__/stockView?id=<?php echo ($stock["id"]); ?>"><?php echo ($stock["stockname"]); ?></a> -
					盤點紀錄
				</div>
				<form id="form1" name="form1" method="post"
					action="__URL__/stockInventorySave?id=<?php echo ($stock["id"]); ?>">
					<input type="hidden" name="inventoryuser"
						value="<?php echo $_SESSION["SESSION_USER"]["name"];?>(<?php echo $_SESSION["SESSION_USER"]["ucode"];?>)" />
				...
				<table width="695" border="0" cellpadding="0" cellspacing="0"
					...
					<td align="center"><?php echo (date('Y/m/d',strtotime($item["startdate"]))); ?> <?php echo ($item["stockclosehour"]); ?>:<?php echo ($item["stockcloseminute"]); ?></td>
					<td align="center"><?php echo (formatDate($item["enddate"])); ?> <?php echo ($item["stockopenhour"]); ?>:<?php echo ($item["stockopenminute"]); ?></td>
					<td align="center"><?php echo ($item["inventoryuser"]); ?></td>
					<td align="center"><a href="__URL__/stockInventoryView?id=<?php echo ($item["id"]); ?>&stockid=<?php echo ($stock["id"]); ?>">明細</a></td>
