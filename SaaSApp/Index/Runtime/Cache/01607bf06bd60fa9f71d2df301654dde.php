...
<script type="text/javascript">
	function check() {
		return validator.empty('name', '請輸入類別名稱') && validator.index('type', '請輸入類別名稱');
	}
</script>
</head>
...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">工作日誌 - 類別管理</div>
			<div class="mainInfo">
				<form action="__URL__/<?php echo ($action); ?>" method="post" onsubmit="return check();" enctype="multipart/form-data" name="form1" id="form1">
					...
					<table width="695" border="0" align="center" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="110" scope="row">子類別名稱</th>
							<td width="587"><select name="type" id="type">
									<option value="選擇主類別" selected="selected">選擇主類別</option>
									<?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(!empty($type)): ?><?php if(($item["id"])  ==  $type["type"]): ?><option selected="selected" value="<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></option>
									<?php else: ?>
									<option value="<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></option><?php endif; ?> <?php else: ?>
									<option value="<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></option><?php endif; ?><?php endforeach; endif; else: echo "" ;endif; ?>
							</select> <input type="text" name="name" id="name" value="<?php echo ($type["name"]); ?>" /> <input type="submit" name="Submit" value="確定" /></td>
						</tr>
					</table>
				</form>
				<form id="form3" name="form3" method="post" action="">
					...
					<div class="mailNav">
						<a href="__URL__/category">主類別列表</a> <select name="stype">
							<option value="" selected="selected">選擇主類別</option>
						...
						</select> <input type="text" name="textfield22" /> <input type="submit" name="Submit3" value="搜尋" />
					</div>
				</form>
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					<tr>
						<th width="99" scope="col">代碼</th>
						<th width="254" scope="col">主類別名稱</th>
						<th width="262" scope="col">子類別</th>
						<th width="78" scope="col">操作</th>
					</tr>
					...
					<td align="center"><?php echo ($item["ccode"]); ?></td>
					<td align="center"><?php echo ($item["mname"]); ?></td>
					<td align="center"><?php echo ($item["name"]); ?></td>
					<td align="center"><a href="__URL__/editchildcategory?id=<?php echo ($item["id"]); ?>">修改</a> | <a onclick="return confirm('確實要刪除選擇的數據嗎？');" href="__URL__/deletechildcategory?id=<?php echo ($item["id"]); ?>">刪除</a></td>
