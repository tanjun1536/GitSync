...
$(function(){
	...
	$("input[name='invoicetype'][value='<?php echo ($invoice["invoicetype"]); ?>']").attr('checked',true);
	$("input[name='estimatedelegatemethods'][value='<?php echo ($invoice["estimatedelegatemethods"]); ?>']").attr('checked',true);
...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
			...
			<div class="mainInfo">
				<form id="form1" name="form1" method="post"
					...
					<input name="code" type="hidden" value="<?php echo $_SESSION["SESSION_USER"]["code"];?>" />
					...
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						...
						<tr>
							...
							<input type="hidden" id="reqdate" name="reqdate" value="<?php echo ($invoice["reqdate"]); ?>"  />
							<td width="562" id="tdreqdate">2013/04/29</td>
						</tr>
						<tr>
							<th width="128" scope="row">對應報價單號</th>
							<td><input type="text" name="pricecode" value="<?php echo ($invoice["pricecode"]); ?>" /></td>
						</tr>
						<tr>
						<input type="hidden" id="requestid" name="requestid" value="<?php echo $_SESSION["SESSION_USER"]["id"];?>"  />
						<input type="hidden" id="requestname" name="requestuser" value="<?php echo $_SESSION["SESSION_USER"]["name"];?>"  />
							<th scope="row">申請人</th>
							<td><?php echo $_SESSION["SESSION_USER"]["name"];?></td>
						</tr>
						<tr>
							<th scope="row">申請單位</th>
							<input type="hidden" id="requestunitid" name="requestunitid" value="<?php echo ($invoice["requestunitid"]); ?>"  />
							<input type="hidden" id="requestunitname" name="requestunitname" value="<?php echo ($invoice["requestunitname"]); ?>"  />
							<td><?php echo ($invoice["requestunitname"]); ?></td>
						</tr>
						<tr>
					...
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						...
						<?php if(is_array($details)): $i = 0; $__LIST__ = $details;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($i%2 == 1): ?><tr>
							...
							<td><?php echo ($item["productname"]); ?><input type="hidden" name="details[]" value="<?php echo ($item["productname"]); ?>,<?php echo ($item["productcount"]); ?>,<?php echo ($item["productprice"]); ?>,<?php echo ($item["totalprice"]); ?>" /> </td>
						<td align="center"><?php echo ($item["productcount"]); ?></td>
						<td align="center"><?php echo ($item["productprice"]); ?></td>
						<td align="center"><?php echo ($item["totalprice"]); ?></td>
					...
					<div>
						<p>
						<input type="hidden" id="total" name="total" value="<?php echo ($invoice["total"]); ?>" />
					<input type="hidden" id="tax" name="tax" value="<?php echo ($invoice["tax"]); ?>" />
					<input type="hidden" id="totalprice" name="totalprice" value="<?php echo ($invoice["totalprice"]); ?>" />
			
							發票金額合計：<span id="stotal" ><?php echo ($invoice["total"]); ?></span><br /> 稅金(5%)：<span id="stax"><?php echo ($invoice["tax"]); ?></span><br /> 發票金額總計：<span id="stotalprice"><?php echo ($invoice["totalprice"]); ?></span><br />
						</p>
					</div>
					<div align="center">
