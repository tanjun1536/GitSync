...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">廠商管理 - 廠商分類管理</div>
			<div class="mainInfo">
				<form action="__URL__/addManufacturersCategory" onsubmit="return check()" method="post" enctype="multipart/form-data" name="form1" id="form1">
				...
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					<tr>
						<th width="349" scope="col">分類名稱</th>
					...
					<td align="center"><a href="__URL__/updateManufacturersCategory?id=<?php echo ($item["id"]); ?>">修改</a> <a href="__URL__/deleteManufacturersCategory?id=<?php echo ($item["id"]); ?>" onclick="return confirm('是否刪除該數據?')">刪除 </a></td>
