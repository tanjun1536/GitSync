...
<script type="text/javascript">
	var dept = "<?php echo ($price["queryerdepart"]); ?>";
	var team = "<?php echo ($price["queryerteam"]); ?>";
	var queryer = "<?php echo ($price["queryer"]); ?>";
	var reqtype = "<?php echo ($price["reqtype"]); ?>";
	$(function() {
		...
		if(dept)$('#queryerdepart').val(dept).trigger('change');
		if(reqtype)$("#reqtype").val(reqtype);
	});
	function getTeam(depart, teams) {
		$.ajax({
			...
			success : function(data) {
				...
				if(team)$(teams).val(team).trigger('change');
			}
		});
	}
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				...
				<input type="hidden" name="id" value="<?php echo ($price["id"]); ?>" /> 
					<input type="hidden" name="code" value="<?php echo $_SESSION["SESSION_USER"]["code"];?>" /> <input type="hidden"
						name="querydate" id="querydate" value="<?php echo ($price["querydate"]); ?>" /> <input type="hidden"
						name="querycode" id="querycode" value="<?php echo ($price["querycode"]); ?>" /> <input type="hidden"
						name="state" value="未處理" /> <input type="hidden" name="totaldj" id="totaldj"
						value="<?php echo ($price["totaldj"]); ?>" /> <input type="hidden" name="totalfj" id="totalfj"
						value="<?php echo ($price["totalfj"]); ?>" />
					<div class="topic2">新增／修改詢價單</div>
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="116" scope="row">日期</th>
							<td width="228" id="tdquerydate"><?php echo (date('Y/m/d',strtotime($price["querydate"]))); ?></td>
							<th width="100" scope="row">編號</th>
							<td width="246" id="tdquerycode"><?php echo ($price["querycode"]); ?></td>
						</tr>
						<tr>
							<th scope="row">詢價人</th>
							<td colspan="3" id="tdqueryer">
								<?php echo ($price["queryername"]); ?>
...
<!-- 							</select> -->
						...
						<tr>
							...
							<td colspan="3"><select name="reqtype" id="reqtype">
						...
						<tr>
							<th scope="row">狀態</th>
							<td colspan="3"><?php echo ($price["state"]); ?></td>
						</tr>
						<tr>
							<th scope="row">採購說明</th>
							<td colspan="3"><input type="text" name="purchaseremark" value="<?php echo ($price["purchaseremark"]); ?>"
								style="width: 400px" /></td>
						</tr>
						<tr>
							<th scope="row">備註說明</th>
							<td colspan="3"><input type="text" name="remark" value="<?php echo ($price["remark"]); ?>"
								style="width: 400px" /></td>
						</tr>
					</table>
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
					...
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide" id="tabCP">
						...
						<?php if(is_array($details)): $i = 0; $__LIST__ = $details;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><tr>
							<td><?php echo ($item["cp"]); ?></td>
							<td><?php echo ($item["spmc"]); ?></td>
							<td><?php echo ($item["ggcc"]); ?></td>
							<td><?php echo ($item["cpxh"]); ?></td>
							<td><?php echo ($item["sl"]); ?></td>
							<td><?php echo ($item["dw"]); ?></td>
							<td><?php echo ($item["dj"]); ?></td>
							<td><?php echo ($item["fj"]); ?></td>
							<td><?php echo ($item["bz"]); ?></td>
							<td><img onclick="editCP($(this).parent().parent());" style="cursor:pointer" src="__PUBLIC__/images/icon_edit.gif" alt="編輯" width="16" height="16" border="0" /><img onclick="$(this).parent().parent().remove();cal();" style="cursor:pointer" src="__PUBLIC__/images/close_16.png" alt="刪除" width="16" height="16" border="0" /></td>
							<input type="hidden" name="detail[]"
								value="<?php echo ($item["cp"]); ?>,<?php echo ($item["spmc"]); ?>,<?php echo ($item["ggcc"]); ?>,<?php echo ($item["cpxh"]); ?>,<?php echo ($item["sl"]); ?>,<?php echo ($item["dw"]); ?>,<?php echo ($item["dj"]); ?>,<?php echo ($item["fj"]); ?>,<?php echo ($item["bz"]); ?>" />
						</tr><?php endforeach; endif; else: echo "" ;endif; ?>
						<tr>
							<td colspan="6" align="right" class="tdcolor1">總計</td>
							<td align="center" id="tdTotalDJ"><?php echo ($price["totaldj"]); ?></td>
							<td align="center" id="tdTotalFJ"><?php echo ($price["totalfj"]); ?></td>
