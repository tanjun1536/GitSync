...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="mailNav">
					<a href="__URL__/create">寫信</a><a href="__URL__/send"> 寄件備份</a><a href="__URL__/index" class="select">收件夾(<strong><?php echo $_SESSION["MSG_COMM"];?></strong>)
					</a>
				</div>
				<div class="mailNav2">
					<div>
						<a href="__URL__/response?id=<?php echo ($msg["id"]); ?>">回覆</a><a href="__URL__/sendtoothers?id=<?php echo ($msg["id"]); ?>">轉寄</a>
					</div>
					<table width="683" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td bgcolor="#D2ECFB"><strong>寄件者 :</strong></td>
							<td width="458" bgcolor="#D2ECFB"><?php echo ($msg["sendername"]); ?></td>
							<td width="170" align="right" bgcolor="#D2ECFB"><?php echo (date('y/m/d h:m A',strtotime($msg["senddate"]))); ?></td>
						</tr>
						<tr>
							<td width="55" bgcolor="#D2ECFB"><strong>收件者 :</strong></td>
							<td colspan="2" bgcolor="#D2ECFB"><?php echo ($msg["receiversname"]); ?></td>
						</tr>
						<tr>
							<td bgcolor="#D2ECFB"><strong>主 旨 :</strong></td>
							<td colspan="2" bgcolor="#D2ECFB">Re:<?php echo ($msg["title"]); ?></td>
						</tr>
					</table>
				</div>
				<?php if(!empty($msg["atts"])): ?><div class="mailFile">
					<input type="hidden" value="<?php echo ($msg["atts"]); ?>" name="atts"> </input> [ <a href="__URL__/zipdown?names=<?php echo ($msg["atts"]); ?>">儲存全部附件</a> ] <?php $_result=explode(',',$msg['atts']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$att): ++$i;$mod = ($i % 2 )?><a href="__URL__/down?name=<?php echo (urlencode($att)); ?>" target="_blank"><?php echo ($att); ?></a>；<?php endforeach; endif; else: echo "" ;endif; ?>
				</div><?php endif; ?>
				<div><?php echo ($msg["cont"]); ?></div>
				<?php if(!empty($orign)): ?><div class="mail_info">
					<div><?php echo ($orign["cont"]); ?></div>
					<div class="mail_orig">
						----- Original Message ----- <br /> <strong>寄件者：</strong><?php echo ($orign["sendername"]); ?><br /> 
						<strong>收件者：</strong><?php echo ($orign["receiversname"]); ?> <br /> 
						<strong>寄件日期：</strong><?php echo (date('y/m/d h:m A',strtotime($orign["senddate"]))); ?> <br /> 
						<strong>主旨：</strong><?php echo ($orign["title"]); ?><br />
					</div>
					<div><?php echo ($orign["cont"]); ?></div>
				</div><?php endif; ?>
			</div>
