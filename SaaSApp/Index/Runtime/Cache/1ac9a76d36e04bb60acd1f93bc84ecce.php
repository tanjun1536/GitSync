...
<script type="text/javascript">
	...
	function S(btn) {
		...
		if (btn.value != '確認') {
			frm.action = "__URL__/deleteScheduling?case=<?php echo $_SESSION["case"];?>";
		} else {
			frm.action = "__URL__/sureScheduling?case=<?php echo $_SESSION["case"];?>";
		}
		frm.submit();
	}
</script>
...
<!-- 				</ul></li> -->
  ...
  <?php echo $_SESSION["caseName"];?>
			...
			<div class="mainInfo">
				...
				<form id="form1" onsubmit="return check()" name="form1" method="post" action="__URL__/addScheduling?case=<?php echo $_SESSION["case"];?>">
					<input type="hidden" name="case" value="<?php echo $_SESSION["case"];?>" /> <input type="hidden" name="state" value="新增" />
				...
				<form id="form2" name="form2" method="post">
					...
					<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
						...
						<td align="center"><a href="__URL__/schedulingCheck?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>" target="_blank"><?php echo ($item["year"]); ?>/<?php echo ($item["month"]); ?></a></td>
						<td align="center"><?php echo ($item["state"]); ?></td>
						<td align="center">
									<?php if(check_power('核對',$ext) == 1): ?><a href="__URL__/schedulingCheck?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>" target="_blank"> 保養核對 </a><?php endif; ?>
										
						|<?php if(check_power('匯出',$ext) == 1): ?><a href="__URL__/exportScheduling?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>"> 匯出</a><?php endif; ?></td>
