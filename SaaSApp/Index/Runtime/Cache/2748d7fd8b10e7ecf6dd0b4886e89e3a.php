...
<script type="text/javascript">
	var billprov = "<?php echo ($customer["billprov"]); ?>";
	var clientprov = "<?php echo ($customer["clientprov"]); ?>";
	var deliverprov = "<?php echo ($customer["deliverprov"]); ?>";
	var billcity = "<?php echo ($customer["billcity"]); ?>";
	var clientcity = "<?php echo ($customer["clientcity"]); ?>";
	var delivercity = "<?php echo ($customer["delivercity"]); ?>";
	var hytype = "<?php echo ($customer["hytype"]); ?>";
	var billcity=null;
	...
	$(function() {
		...
		$("input[name='billtype'][value='<?php echo ($customer["billtype"]); ?>']").attr('checked', true);
		$("input[name='membergrade'][value='<?php echo ($customer["membergrade"]); ?>']").attr('checked', true);
		$("input[name='paymanner'][value='<?php echo ($customer["paymanner"]); ?>']").attr('checked', true);
		$("input[name='billmanner'][value='<?php echo ($customer["billmanner"]); ?>']").attr('checked', true);
	});
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<form action="__URL__/saveCustomer" method="post" enctype="multipart/form-data" name="form1" id="form1">
					<input type="hidden" name="code" value="<?php echo $_SESSION["SESSION_USER"]["code"];?>" /> <input type="hidden" name="id" value="<?php echo ($customer["id"]); ?>" />
					<div class="topic2">新增 / 修改客戶</div>
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th scope="row">客戶編號</th>
							<td><input type="text" name="clientcode" value="<?php echo ($customer["clientcode"]); ?>" /></td>
						</tr>
						<tr>
							<th width="131" scope="row">公司抬頭</th>
							<td width="561"><input type="text" name="clienttitle" value="<?php echo ($customer["clienttitle"]); ?>" style="width: 300px" /></td>
						</tr>
						<tr>
							<th width="161" scope="row">客戶名稱</th>
							<td width="531"><input type="text" name="companyname"  value="<?php echo ($customer["companyname"]); ?>" style="width: 300px" /></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><input type="text" name="uniquecode" value="<?php echo ($customer["uniquecode"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">公司電話</th>
							<td>電話一 <input type="text" name="phone1" value="<?php echo ($customer["phone1"]); ?>" /> ；電話二 <input type="text" name="phone2" value="<?php echo ($customer["phone2"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">傳真</th>
						...
						<tr>
							...
							<td><select name="billprov" id="billprov">
									...
									<option value="請選擇">請選擇</option>
							</select> <input type="text" id="billaddress" name="billaddress" value="<?php echo ($customer["billaddress"]); ?>"  style="width: 300px" /></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><select name="clientprov" id="clientprov">
									...
									<option value="請選擇">請選擇</option>
							</select> <input type="text" id="clientaddress" name="clientaddress" value="<?php echo ($customer["clientaddress"]); ?>" style="width: 300px" />
						...
						<tr>
							...
							<td><select name="deliverprov" id="deliverprov">
									...
									<option value="請選擇">請選擇</option>
							</select> <input type="text" id="deliveraddress" name="deliveraddress" value="<?php echo ($customer["deliveraddress"]); ?>" style="width: 300px" />
							 <input type="button" onclick="copydeliver()" value="同上" /></td>
						</tr>
						<tr>
							<th scope="row">送貨案場</th>
							<td><input type="text" name="delivercase" value="<?php echo ($customer["delivercase"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">聯絡人一</th>
							<td>姓名 <input type="text" name="linkername1" value="<?php echo ($customer["linkername1"]); ?>" /> ；職位 <input type="text" name="linkerduty1" value="<?php echo ($customer["linkerduty1"]); ?>" /> <br /> 電話 <input type="text" name="linkerphone1" value="<?php echo ($customer["linkerphone1"]); ?>" /> ；手機 <input type="text" name="linkermobile1" value="<?php echo ($customer["linkermobile1"]); ?>" /> <br /> Email <input type="text" name="linkeremail1" value="<?php echo ($customer["linkeremail1"]); ?>" />；部門 <input type="text" name="unit1" value="<?php echo ($customer["unit1"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row"><p>聯絡人二</p></th>
							<td>姓名 <input type="text" name="linkername2" value="<?php echo ($customer["linkername2"]); ?>" /> ；職位 <input type="text" name="linkerduty2" value="<?php echo ($customer["linkerduty2"]); ?>" /> <br /> 電話 <input type="text" name="linkerphone2" value="<?php echo ($customer["linkerphone2"]); ?>" /> ；手機 <input type="text" name="linkermobile2" value="<?php echo ($customer["linkermobile2"]); ?>" /> <br /> Email <input type="text" name="linkeremail2" value="<?php echo ($customer["linkeremail2"]); ?>" />；部門 <input type="text" name="unit2" value="<?php echo ($customer["unit2"]); ?>"/></td>
						</tr>
						<tr>
							<th scope="row">付款銀行</th>
							<td><input type="text" name="paybank" value="<?php echo ($customer["paybank"]); ?>"/></td>
						</tr>
						<tr>
							<th scope="row">所屬分行</th>
							<td><input type="text" name="belongbank" value="<?php echo ($customer["belongbank"]); ?>"/></td>
						</tr>
						<tr>
							<th scope="row">銀行帳號</th>
							<td><input type="text" name="bankcode" value="<?php echo ($customer["bankcode"]); ?>"/></td>
						</tr>
						<tr>
