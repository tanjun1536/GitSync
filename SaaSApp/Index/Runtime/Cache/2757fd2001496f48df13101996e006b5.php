...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
			...
			<div class="mainInfo">
				<form action="__URL__/acceptCheckSave" onsubmit="return check()"
					method="post" enctype="multipart/form-data" name="form1" id="form1">
					<input type="hidden" name="accept" value="<?php echo $_GET["accept"];?>" /> <input
						type="hidden" name="acceptusers" id="acceptusers" />
					<div class="topic2">新增點交紀錄</div>
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th width="111" scope="row">日期</th>
							<td width="581"><input type="text" name="acceptdate"
								class="Wdate" onfocus="WdatePicker()" /></td>
						</tr>
						<tr>
							<th scope="row">工程名稱</th>
							<td><input type="text" name="acceptproject"
								style="width: 400px" /></td>
						</tr>
						<tr>
							<th scope="row">地點</th>
							<td><input type="text" name="acceptaddress"
								style="width: 400px" /></td>
						</tr>
						<tr>
							<th scope="row">點交類型</th>
							<td><input name="accepttype" type="radio" value="自驗"
								checked="checked" /> 自驗 <input name="accepttype" type="radio"
								value="委外" /> 委外</td>
						</tr>
						<tr>
							<th scope="row">內容說明</th>
							<td><textarea name="acceptcontent" rows="5"
									style="width: 500px"></textarea></td>
						</tr>
						<tr>
							<th scope="row">上傳圖片</th>
							<td><ul id="fileContainer">
									<li><input type="file" name="atts[]" /><a
										href="javascript:void(0)" onclick="addFile()"> <img
											src="__PUBLIC__/images/add_16.png" alt="增加" width="16"
											height="16" border="0" /></a></li>
								</ul></td>
						</tr>
						<tr>
							<th scope="row">參與人員</th>
							<td><input type="text" name="acceptuser" id="acceptuser" />
								<a href="javascript:void(0)" onclick="addUser()"><img
									src="__PUBLIC__/images/add_16.png" alt="增加" width="16"
									height="16" border="0" /></a>
								<div>
									<ul id="users">
										<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><li><span><?php echo ($item["name"]); ?></span><a href="javascript:void(0)"
											onclick="$(this).parent().remove();"><img
												src="__PUBLIC__/images/close_16.png" alt="刪除" width="16"
												height="16" border="0" /></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
									</ul>
								</div></td>
						</tr>
						<tr>
