...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="topic2">審核出庫單</div>
				<form id="form1" name="form1" method="post"
					action="__URL__/stockChangeCheck">
					<input type="hidden" name="id" value="<?php echo ($sc["id"]); ?>" /> <input
						type="hidden" name="state" id="state" /> <input type="hidden"
						name="checkuser" id="checkuser" value="<?php echo $_SESSION["SESSION_USER"]["name"];?>" /> <input
						type="hidden" name="checkdate" id="checkdate" />
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th width="91" scope="row">倉庫代號</th>
							<td width="242"><?php echo ($sc["outwarehousecode"]); ?></td>
							<th width="91" scope="row">倉庫名稱</th>
							<td width="266"><?php echo ($sc["outwarehousename"]); ?></td>
						</tr>
						<tr>
							<th scope="row">出庫日期</th>
							<td><?php echo (date('Y/m/d',strtotime($sc["outdate"]))); ?></td>
							<th scope="row">出庫性質</th>
							<td><?php echo ($sc["outtypename"]); ?></td>
						</tr>
						<tr>
							<th scope="row">出庫人</th>
							<td><?php echo ($sc["outuser"]); ?></td>
							<th scope="row">出庫時間</th>
							<td><?php echo ($sc["outhour"]); ?>時<?php echo ($sc["outminute"]); ?>分</td>
						</tr>
						<tr>
							<th scope="row">審核人</th>
							<td><?php echo ($sc["checkuser"]); ?></td>
							<th scope="row">審核日期</th>
							<td><?php if(!empty($sc["checkdate"])): ?><?php echo (date('Y/m/d',strtotime($sc["checkdate"]))); ?><?php endif; ?></td>
						</tr>
						<tr>
							<th scope="row">出貨單號</th>
							<td colspan="3"><?php echo ($sc["outcode"]); ?></td>
						...
						<tr>
							...
							<td><?php echo ($sc["ordercode"]); ?></td>
						</tr>
						<tr>
							<th scope="row">備註</th>
							<td colspan="3"><?php echo ($sc["remark"]); ?></td>
						</tr>
					</table>
					<table width="695" border="0" cellpadding="0" cellspacing="0"
						class="tab_thtop">
						<tr>
							<th width="144" scope="col">商品編號</th>
							<th width="205" scope="col">商品名稱</th>
							<th width="69" scope="col">單價</th>
							<th width="47" scope="col">數量</th>
							<th width="75" scope="col">小計</th>
							<th width="153" scope="col">備註</th>
						</tr>
						<?php if(is_array($scd)): $i = 0; $__LIST__ = $scd;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($i%2 == 1): ?><tr>
							<?php else: ?>
							<tr class="trcolor"><?php endif; ?>
						<td align="center"><?php echo ($item["productcode"]); ?></td>
						...
						<tr class="trcolor3">
							...
							<td align="center"><?php echo ($sc["totalmoney"]); ?></td>
							<td>元</td>
						</tr>
					</table>
					<table width="695" border="0" cellpadding="0" cellspacing="0"
						...
						<tr>
							<td><textarea name="remark" rows="5" style="width: 680px"><?php echo ($sc["remark"]); ?></textarea></td>
						</tr>
					</table>
					<div align="center" style="margin-top: 10px">
						<input type="button" name="Submit" onclick="Check(this)"
							value="核可" /> <input type="button" onclick="Check(this)"
							name="Submit2" value="不同意" />
					</div>
				</form>
			</div>
		</div>
