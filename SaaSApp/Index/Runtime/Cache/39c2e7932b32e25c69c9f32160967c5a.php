...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">客戶管理 - 行業別管理</div>
			<div class="mainInfo">
				<form action="__URL__/addCustomerType" method="post" enctype="multipart/form-data" name="form1" id="form1">
					<div class="topic2">行業別</div>
					<table width="695" border="0" align="center" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="161" scope="row">新增 / 修改 行業別</th>
							<td width="531">
							<input type="text" name="name" value="<?php echo ($ct["name"]); ?>"/>
				...
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					<tr>
						<th width="349" scope="col">行業名稱</th>
					...
					<td align="center"><a href="__URL__/editCustomerType?id=<?php echo ($item["id"]); ?>">修改</a> | <a onclick="return confirm('確認刪除數據?')" href="__URL__/deleteCustomerType?id=<?php echo ($item["id"]); ?>">刪除</a></td>
