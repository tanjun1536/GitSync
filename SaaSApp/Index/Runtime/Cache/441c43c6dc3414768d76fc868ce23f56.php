...
<body>
	<div class="txt_w" style="margin: 10px 20px 20px">
		<strong><?php echo ($year); ?> 年 <?php echo ($month); ?> 月</strong> 月完工數量統計表
	</div>
	...
	<div style="margin: 0px 10px">
		<table border="0" cellpadding="0" cellspacing="1" class="tab_month">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><tr>
				...
				<td><?php echo ($item["day30"]); ?></td>
				<td><?php echo ($item["day31"]); ?></td>
				<td class="tdcolor2"><a target="_blank" href="__URL__/statisticscompletedetail?case=<?php echo $_SESSION["case"];?>&year=<?php echo ($year); ?>&month=<?php echo ($month); ?>&maintype=<?php echo ($item["mt"]); ?>&childtype=<?php echo ($item["ct"]); ?>"><?php echo ($item["rt"]); ?></a></td>
