...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
			...
			<div class="mainInfo">
				<form action="__URL__/aggree" method="post"
					...
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						...
						<tr>
							...
							<td><label><span class="txt_green">[
										<?php echo ($elec["typename"]); ?> ] </span><?php echo ($elec["typechildname"]); ?></label></td>
						</tr>
						<tr>
							<th width="94" scope="row">起迄時間</th>
							<td width="598"><?php echo (date('y/m/d',strtotime($elec["sdate"]))); ?>
								<?php echo ($elec["shour"]); ?>:<?php echo ($elec["sminute"]); ?> ~
								<?php echo (date('y/m/d',strtotime($elec["edate"]))); ?>
								<?php echo ($elec["ehour"]); ?>:<?php echo ($elec["eminute"]); ?></td>
						</tr>
						<tr>
							<th scope="row">金額</th>
						...
						<tr>
							<th scope="row">簽核人</th>
							<td><ul>
									<?php if(is_array($flows)): $i = 0; $__LIST__ = $flows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><li><?php echo (csubstr($item["checkername"],0,strpos($item["checkername"],")")+1)); ?> <span
										class="txt_orang"><?php echo (csubstr($item["checkername"],strripos($item["checkername"],"(")+1,strripos($item["checkername"],")")-strripos($item["checkername"],"(")-1)); ?>
									</span> <?php if(!empty($item["checkerresult"])): ?><?php if($item["checkerresult"] == '同意'): ?><span
											class="txt_green"><?php else: ?><span class="txt_red"><?php endif; ?>(<?php echo ($item["checkerresult"]); ?>)</span></li><?php endif; ?><?php endforeach; endif; else: echo "" ;endif; ?>

								</ul></td>
						</tr>
						<tr>
							<th scope="row">附件</th>
							<td><?php if(($elec["atts"])  !=  ""): ?><?php $_result=explode(',',$elec['atts']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$att): ++$i;$mod = ($i % 2 )?><a
					...
					<input type="hidden" name="id" value="<?php echo ($elec["id"]); ?>" />
				</form>
			</div>
		</div>
	</div>
