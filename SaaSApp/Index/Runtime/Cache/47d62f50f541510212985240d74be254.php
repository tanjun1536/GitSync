...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">提醒 - 過去紀錄</div>
			<div class="mainInfo">
				<form id="form1" name="form1" method="post" action="">
					<div class="mailNav">
						日期<input type="text" name="sdate" class="Wdate"
							onfocus="WdatePicker()" /> 
				...
				<table width="695" border="0" cellpadding="0" cellspacing="0"
					class="tab_thtop">
					<tr>
						<th width="122" scope="col">日期</th>
						<th width="96" scope="col">開始時間</th>
						<th width="90" scope="col">結束時間</th>
						<th width="385" scope="col">內容</th>
					</tr>
					...
					<td align="center"><?php echo (date('Y-m-d',strtotime($item["date"]))); ?>
						(<?php echo (getWeek($item["date"])); ?>)</td>
					<td align="center"><?php echo ($item["shour"]); ?>:<?php echo ($item["sminute"]); ?></td>
					<td align="center"><?php echo ($item["ehour"]); ?>:<?php echo ($item["eminute"]); ?></td>
					<td><a href="__APP__/work/view?id=<?php echo ($item["id"]); ?>"><?php echo (getCut($item["description"])); ?></a></td>
