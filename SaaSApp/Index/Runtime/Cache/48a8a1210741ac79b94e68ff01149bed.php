...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="mailNav">
					<a href="__URL__/AddFavorite">設定我的最愛</a>
				</div>
				<table width="100%" border="0" cellpadding="0" cellspacing="1"
					...
					<tr>
						<td valign="top"><dl style="margin-top: 10px">
								<?php if(is_array($topMenus)): $i = 0; $__LIST__ = $topMenus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): ++$i;$mod = ($i % 2 )?><dt>
									<?php if(empty($menu["link"])): ?><?php echo ($menu["name"]); ?> <?php else: ?> <a
										href="<?php echo ($menu["link"]); ?>"><?php echo ($menu["name"]); ?></a><?php endif; ?>

								</dt>
								<dd>
									<?php if(is_array($menu["children"])): $i = 0; $__LIST__ = $menu["children"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): ++$i;$mod = ($i % 2 )?><a
										href="<?php echo ($child["link"]); ?>"><?php echo ($child["name"]); ?></a>| <?php if(!empty($child["children"])): ?>(<?php if(is_array($child["children"])): $i = 0; $__LIST__ = $child["children"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cchild): ++$i;$mod = ($i % 2 )?><a href="<?php echo ($cchild["link"]); ?>"><?php echo ($cchild["name"]); ?></a>|<?php endforeach; endif; else: echo "" ;endif; ?> )<br /><?php endif; ?><?php endforeach; endif; else: echo "" ;endif; ?>
