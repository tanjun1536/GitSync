...
<!-- 				</ul></li> -->
  ...
  <?php echo $_SESSION["caseName"];?>
	...
	<a href="__URL__/goMaintenanceOverTime?case=<?php echo $_SESSION["case"];?>">新增保養週期</a>
							</if
				...
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					...
					<td align="center"><a href="__URL__/maintenanceOverTimeView?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></a></td>
					<td align="center"><?php echo (getName($item["type"])); ?></td>
					<td align="center">
					<if condition="check_power('修改',$ext) eq 1">
<a href="__URL__/editMaintenanceOverTime?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>">修改 </a>
							</if
							
					| 
					<if condition="check_power('删除',$ext) eq 1">
<a onclick="return confirm('是否删除选择的数据?')" href="__URL__/deleteMaintenaceOverTime?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>">刪除</a>
							</if
							</td>
