...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">資源分享 - 常用表單</div>
			<div class="mainInfo">
				<div style="margin-bottom: 10px"></div>
				<form id="form2" name="form2" method="post" action="__URL__/addCategory">
					<div class="topic2">新增 / 修改類別</div>
					<?php if(($formtype)  !=  ""): ?><<input type="hidden" name="id" value="<?php echo ($formtype["id"]); ?>" id="id" /><?php endif; ?>
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="142" valign="top" scope="row">類別名稱</th>
							<td width="550"><input type="text" name="name" value="<?php echo ($formtype["name"]); ?>" /> <input type="submit" name="Submit" value="確定" /></td>
						</tr>
					</table>
					<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
						<tr>
							<th width="550" scope="col">類別名稱</th>
							<th width="143" scope="col">操作</th>
						</tr>
						...
						<td align="center"><a href="__URL__/edit?id=<?php echo ($item["id"]); ?>">修改</a> | <a onclick="return confirm('是否刪除該數據?')" href="__URL__/deleteCategory?id=<?php echo ($item["id"]); ?>">刪除</a></td>
