...
<script type="text/javascript">
	$(function() {
		...
		$("#tab_Detail input[type='text']").live('keyup',function(){
			$("#tdTotal").html(
					fn.getFloat($("#m1").val())+
					fn.getFloat($("#m2").val())+
					fn.getFloat($("#m3").val())+
	...
	function addDetail() {
		if (
				validator.index('subject', '請選擇部門!')&&
				validator.empty('m1', '請輸入1月預算!')&&
				validator.empty('m2', '請輸入2月預算!')&&
				validator.empty('m3', '請輸入3月預算!')&&
				validator.empty('m4', '請輸入4月預算!')&&
				validator.empty('m5', '請輸入5月預算!')&&
				validator.empty('m6', '請輸入6月預算!')&&
				validator.empty('m7', '請輸入7月預算!')&&
				validator.empty('m8', '請輸入8月預算!')&&
				validator.empty('m9', '請輸入9月預算!')&&
				validator.empty('m10', '請輸入10月預算!')&&
				validator.empty('m11', '請輸入11月預算!')&&
				validator.empty('m12', '請輸入12月預算!')
				
		) {
			var table = $('#tab_Detail');
			var len = $("#tab_Detail tr").length;
			var row = len % 2 == 0 ? $("<tr class=\"trcolor\"></tr>") : $("<tr></tr>");
			var hValue = $('#subject').find("option:selected").text() + "," 
			+ $('#m1').val() + ","
			+ $('#m2').val() + ","
			+ $('#m3').val() + ","
			+ $('#m4').val() + ","
...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
			...
			<div class="mainInfo">
				<form action="__URL__/addBudget" method="post" enctype="multipart/form-data" name="form1" id="form1">
					<input type="hidden" name="code" value="<?php echo $_SESSION["SESSION_USER"]["code"];?>" /> <input type="hidden" name="createdate" id="createdate" />
					<div class="topic2">新增 / 修改預算編列資料</div>

					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="88" scope="row">建立日期</th>
							<td width="255" id="tdCreateDate">系統帶出</td>
							<th width="101" scope="row">編列人</th>
							<td width="246"><input type="text" name="createuser" /></td>
						</tr>
						<tr>
					...
					<div align="center" style="margin-top: 10px">
						<input type="submit" name="Submit" value="新增" /> <input type="reset" name="Submit2" value="取消" />
					</div>
				</form>
			</div>
		</div>
