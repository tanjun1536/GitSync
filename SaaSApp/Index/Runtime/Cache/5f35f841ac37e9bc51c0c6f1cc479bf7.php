			...
			<div class="mainInfo">
				<div class="mailNav">
					<form id="form2" name="form2" method="post" action="__URL__/index">
						<a href="__URL__/add">新增會議通知</a> 日期 <input type="text" name="date" class="Wdate" onfocus="WdatePicker()" /> <select name="state">
							<option value="">選擇狀態</option>
							<?php if($state == '會議通知'): ?><option value="會議通知" selected="selected">會議通知</option>
							<?php else: ?>
							<option value="會議通知">會議通知</option><?php endif; ?>
							<?php if($state == '會議記錄'): ?><option value="會議記錄" selected="selected">會議記錄</option>
							<?php else: ?>
							<option value="會議記錄">會議記錄</option><?php endif; ?>
							<?php if($state == '會議取消'): ?><option value="會議取消" selected="selected">會議取消</option>
							<?php else: ?>
							<option value="會議取消">會議取消</option><?php endif; ?>
							<?php if($state == '不拘'): ?><option value="" selected="selected">不拘</option>
							<?php else: ?>
							<option value="">不拘</option><?php endif; ?>
						</select> <input type="submit" name="Submit3" value="搜尋" />
					</form>
				</div>
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					<tr>
						<th width="82" scope="col">開會日期</th>
						<th width="52" scope="col">時間</th>
						<th width="203" scope="col">開會地點</th>
						<th width="203" scope="col">主題</th>
						<th width="82" scope="col">狀態</th>
					...
					<td align="center"><?php echo ($item["hours"]); ?>:<?php echo (zeroFill($item["minutes"])); ?></td>
					<td align="center"><?php echo ($item["address"]); ?></td>
					<td><a href="__URL__/view?id=<?php echo ($item["id"]); ?>"><?php echo ($item["topic"]); ?></a></td>
					<td align="center"><?php echo ($item["state"]); ?></td>
					<td align="center"><a href="__URL__/record?id=<?php echo ($item["id"]); ?>">記錄</a><a href="__URL__/changeState?id=<?php echo ($item["id"]); ?>&state=會議取消">取消</a><br />
					<a onclick="return confirm('是否刪除選擇的數據?')" href="__URL__/delete?id=<?php echo ($item["id"]); ?>">删除</a></td>
