...
<script type="text/javascript">
	var selectDepart="<?php echo ($selectDepart); ?>";
	var selectTeam="<?php echo ($selectTeam); ?>";
	var selectUser="<?php echo ($selectUser); ?>";
	$(function() {
		...
		$('#status').attr('value','<?php echo ($selectStatus); ?>');
...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">工作日誌 - 部門日誌</div>
			<div class="mainInfo">
				<form id="form1" name="form1" method="post" action="">
					<div class="mailNav">
						<select name="depart" id="depart" >
<!-- 							<option value="">所有部門</option> -->
							<?php if(is_array($departs)): $i = 0; $__LIST__ = $departs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(($selectDepart)  ==  $item["id"]): ?><option value="<?php echo ($item["id"]); ?>" selected="selected"><?php echo ($item["name"]); ?></option>
							<?php else: ?>
					...
					<div class="mailNav">
						<strong>日期：</strong> <input type="text" name="sdate" class="Wdate" value="<?php echo ($sdate); ?>" onFocus="WdatePicker()" style="width: 100px" /> ~ <input type="text" name="edate"  value="<?php echo ($edate); ?>" class="Wdate" onFocus="WdatePicker()" style="width: 100px" /> <input type="submit" name="Submit3" value="搜尋" />
					</div>
					<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
						...
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($i%2 == 1): ?><tr><?php else: ?><tr class="trcolor"><?php endif; ?> 
							...
							<td align="center"><?php echo ($item["shour"]); ?>:<?php echo ($item["sminute"]); ?></td>
							<td align="center"><?php echo ($item["ehour"]); ?>:<?php echo ($item["eminute"]); ?></td>
