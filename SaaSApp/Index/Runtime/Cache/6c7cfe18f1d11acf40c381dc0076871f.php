...
<!-- 				</ul></li> -->
  ...
  <?php echo $_SESSION["caseName"];?>
			...
			<div class="topic">抄表管理 - 類別管理</div>
			<div class="mainInfo">
				<div class="topic2">新增／修改抄表資料</div>
				<form action="__URL__/copyFormAdd" method="post" enctype="multipart/form-data" name="form1" id="form1">
					<input type="hidden" name="case" value="<?php echo $_SESSION["case"];?>" />
					<input type="hidden" name="id" value="<?php echo ($copy["id"]); ?>"  />
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="113" scope="row">編號</th>
							<td width="579"><input type="text" name="code" value="<?php echo ($copy["code"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">建物區域</th>
							<td><input type="text" name="area" value="<?php echo ($copy["area"]); ?>"  /></td>
						</tr>
						<tr>
							<th scope="row">公共區域</th>
							<td><input name="ispublic" type="radio" value="是" checked="checked" /> 是 <input name="ispublic" type="radio" value="否" /> 否</td>
						</tr>
						<tr>
							<th scope="row">用戶名</th>
							<td><input type="text" name="username" value="<?php echo ($copy["username"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">錶號</th>
							<td><input type="text" name="gaugecode" value="<?php echo ($copy["gaugecode"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">倍數</th>
							<td><input type="text" name="times" value="<?php echo ($copy["times"]); ?>"/></td>
						</tr>
						<tr>
							<th scope="row">最大值</th>
							<td><input type="text" name="maxvalue" value="<?php echo ($copy["maxvalue"]); ?>"/></td>
						</tr>
						<tr>
							<th scope="row">類別</th>
							<td><input type="text" name="type"  value="<?php echo ($copy["type"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">排序</th>
							<td><input type="text" name="ordernumber" value="<?php echo ($copy["ordernumber"]); ?>"  /></td>
						</tr>
						<tr>
							<th scope="row">備註</th>
							<td><input type="text" name="remark" style="width: 400px" value="<?php echo ($copy["remark"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
