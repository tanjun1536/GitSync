...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">我的最愛</div>
			<div class="mainInfo">
				<form action="__URL__/SaveFavorite" method="post" enctype="multipart/form-data"
					name="form1" id="form1">
					<div class="topic2">設定我的最愛</div>
					<table width="100%" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th scope="col">基本功能</th>
						</tr>
						<tr>
							<td valign="top">
								<dl style="margin-top: 10px">
									<?php if(is_array($topMenus)): $i = 0; $__LIST__ = $topMenus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): ++$i;$mod = ($i % 2 )?><dt>
										<?php if(in_array(($menu["id"]), is_array($hasMenus)?$hasMenus:explode(',',$hasMenus))): ?><input type="checkbox" checked="checked" name="m[]" value="<?php echo ($menu["id"]); ?>" />
										<?php else: ?>
											<input type="checkbox" name="m[]" value="<?php echo ($menu["id"]); ?>" /><?php endif; ?>
										<?php echo ($menu["name"]); ?>
									</dt>
									<dd>
										<?php if(is_array($menu["children"])): $i = 0; $__LIST__ = $menu["children"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): ++$i;$mod = ($i % 2 )?><?php if(in_array(($child["id"]), is_array($hasMenus)?$hasMenus:explode(',',$hasMenus))): ?><input type="checkbox" checked="checked" name="m[]" value="<?php echo ($child["id"]); ?>" />
										<?php else: ?>
											<input type="checkbox" name="m[]" value="<?php echo ($child["id"]); ?>" /><?php endif; ?>
										<?php echo ($child["name"]); ?>
										<?php if(!empty($child["children"])): ?>(<?php if(is_array($child["children"])): $i = 0; $__LIST__ = $child["children"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cchild): ++$i;$mod = ($i % 2 )?><?php if(in_array(($cchild["id"]), is_array($hasMenus)?$hasMenus:explode(',',$hasMenus))): ?><input type="checkbox" checked="checked" name="m[]" value="<?php echo ($cchild["id"]); ?>" />
										<?php else: ?>
											<input type="checkbox" name="m[]" value="<?php echo ($cchild["id"]); ?>" /><?php endif; ?>
											
											 <?php echo ($cchild["name"]); ?><?php endforeach; endif; else: echo "" ;endif; ?> )<br /><?php endif; ?><?php endforeach; endif; else: echo "" ;endif; ?>
						...
						<tr>
							<td align="center" valign="top"><input type="submit"
								name="Submit" value="確定" /> <input type="reset" name="Submit2"
								value="取消" /></td>
						</tr>
					</table>
				</form>
