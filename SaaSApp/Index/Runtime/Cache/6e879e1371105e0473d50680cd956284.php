		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<form action="__URL__/insert" method="post" onsubmit="return check();" enctype="multipart/form-data" name="form1"
					...
					<?php if(($id)  !=  ""): ?><input type="hidden"  name="id" value="<?php echo ($id); ?>"></input><?php endif; ?>
					<table width="695" border="0" align="center" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							...
							<td width="587"><select name="depart" id="depart">
									...
									<?php if(is_array($departs)): $i = 0; $__LIST__ = $departs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($team)): ?><option value="<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></option>
									<?php else: ?> 
									<?php if(($team["depart"])  ==  $item['id']): ?><option value="<?php echo ($item["id"]); ?>" selected="selected"><?php echo ($item["name"]); ?></option>
									<?php else: ?>
									<option value="<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></option><?php endif; ?><?php endif; ?><?php endforeach; endif; else: echo "" ;endif; ?>

							</select> <input type="text" name="name" id="name" value="<?php echo ($team["name"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
				...
				<form id="form2" name="form2" method="post" action="">
					...
					<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
						...
						<?php if(is_array($teams)): $i = 0; $__LIST__ = $teams;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($i%2 == 1): ?><tr><?php else: ?><tr class="trcolor"><?php endif; ?> 
							<td><?php echo ($item["depart"]); ?></td>
							<td align="center"><?php echo ($item["name"]); ?></td>
							<td align="center"><a href="#" onclick="MM_openBrWindow('__APP__/User/friendforteam?team=<?php echo ($item["id"]); ?>','','width=775,height=500')"><?php echo ($item["users"]); ?></a></td>
							<td align="center"><a href="__URL__/edit?id=<?php echo ($item["id"]); ?>">修改</a> | <a onclick="return confirm('是否刪除該數據?');"
								href=" __URL__/delete?id=<?php echo ($item["id"]); ?>">刪除</a></td>
