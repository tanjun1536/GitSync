		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
                ...
                <div class="mainInfo">
                    <form id="form1" name="form1" method="post" action="__URL__/index">
                        <div class="mailNav">
                           <?php if(check_power('類別管理',$ext) == 1): ?><a href="__URL__/category">類別管理</a><?php endif; ?>
                             <?php if(check_power('新增',$ext) == 1): ?><a href="__URL__/add">新增表單</a><?php endif; ?>
                            <select name="type">
                                <option value="">類別搜尋</option>
                                <?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(($type)  ==  $item["id"]): ?><option selected="selected" value="<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></option>
                                        <?php else: ?>
                    ...
                    <form id="form2" name="form2" onsubmit="return delAwoke();" method="post" action="__URL__/delete">
                        <table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
                            ...
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($i%2 == 1): ?><tr>
                                ...
                                <td align="center">
                                <input type="checkbox" name="itemcheckbox[]" value="<?php echo ($item["id"]); ?>" />
                                </td> <td align="center"><?php echo (date('Y/m/d',strtotime($item["senddate"]))); ?></td>
                                <td align="center"><?php echo ($item["typename"]); ?></td>
                                <td><a href="__URL__/down?name=<?php echo (urlencode($item["atts"])); ?>"><?php echo ($item["name"]); ?></a></td>
                                 <td align="center"><?php echo ($item["sendername"]); ?></td>
                                <td align="center"><?php echo (getExtion($item["atts"])); ?></td>
                                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                        </table>
                        <div style="margin-top:10px">
                        <?php if(check_power('刪除',$ext) == 1): ?><input type="submit" name="Submit2" value="刪除" /><?php endif; ?>
