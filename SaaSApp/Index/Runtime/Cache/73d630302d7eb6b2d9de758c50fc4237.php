...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				...
				<div class="mailNav">
				<?php if(check_power('新增驗貨單',$ext) == 1): ?><a href="__URL__/addStockCheckPage">新增驗貨單</a><?php endif; ?>
				...
				<form id="form1" name="form1" method="post" onsubmit="return delAwoke()" action="__URL__/deleteStockCheck">
					<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
						...
						<td align="center"><a href="__URL__/stockCheckView?id=<?php echo ($item["id"]); ?>"><?php echo ($item["checkcode"]); ?></a></td>
					...
					<?php if(check_power('刪除驗貨單',$ext) == 1): ?><input type="submit" name="Submit" value="刪除" /><?php endif; ?>
