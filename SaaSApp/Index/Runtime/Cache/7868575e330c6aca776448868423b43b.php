...
<!-- 				</ul></li> -->
  ...
  <?php echo $_SESSION["caseName"];?>
			...
			<div class="mainInfo">
				<div class="topic2">新增 / 修改主類別</div>
				<form id="form1" name="form1" method="post" action="__URL__/addDeviceType?case=<?php echo $_SESSION["case"];?>">
					<input type="hidden" name="case" value="<?php echo $_SESSION["case"];?>" />
					<input type="hidden" name="id" value="<?php echo ($dt["id"]); ?>" />
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="119" scope="row">類別名稱</th>
							<td width="571"><input type="text" name="name" value="<?php echo ($dt["name"]); ?>" /> <input type="submit" name="Submit" value="確定" /></td>
						</tr>
					</table>
				</form>
				<div class="mailNav">
					<a href="__URL__/deviceChildType?case=<?php echo $_SESSION["case"];?>">子類別列表</a>
				</div>
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					<tr>
						<th width="451" scope="col">主類別</th>
						<th width="242" scope="col">操作</th>
					</tr>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($i%2 == 1): ?><tr><?php else: ?><tr class="trcolor"><?php endif; ?> 
						<td align="center"><a href="__URL__/deviceChildType?case=<?php echo $_SESSION["case"];?>&mtype=<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></a></td>
						<td align="center"><a href="__URL__/editDeviceType?id=<?php echo ($item["id"]); ?>&case=<?php echo $_SESSION["case"];?>">修改</a> | <a onclick="return confirm('是否刪除選擇的數據?')" href="__URL__/deleteDeviceType?id=<?php echo ($item["id"]); ?>&case=<?php echo $_SESSION["case"];?>">刪除</a></td>
