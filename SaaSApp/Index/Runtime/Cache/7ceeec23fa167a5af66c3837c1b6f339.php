...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<form id="form1" name="form1" method="post" action="__URL__/InvoiceList">
					<div class="topic2">搜尋</div>
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th width="104" scope="row">請款日期</th>
							<td width="239"><input type="text" name="sdate"
								class="Wdate" onfocus="WdatePicker()" value="<?php echo ($sdate); ?>" /></td>
							<th width="120" scope="row">對應報價單號</th>
							<td width="227"><input type="text" name="spricecode" value="<?php echo ($spricecode); ?>"  /></td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
					...
					<div class="mailNav">
						<a href="__URL__/InvoiceAdd">新增發票請款記錄</a>
					</div>
				</form>
				<table width="695" border="0" cellpadding="0" cellspacing="1"
					class="tabThSide">
					<tr>
						<th width="98" scope="col">請款日期</th>
						<th width="128" scope="col">對應報價單號</th>
						<th width="71" scope="col">申請人</th>
						<th width="205" scope="col">申請單位</th>
						<th width="98" scope="col">發票總金額</th>
					...
					<tr>
						<td align="center"><?php echo ($item["reqdate"]); ?></td>
						<td align="center"><?php echo ($item["pricecode"]); ?></td>
						<td align="center"><?php echo ($item["requestuser"]); ?></td>
						<td><?php echo ($item["requestunitname"]); ?></td>
						<td align="center"><?php echo ($item["totalprice"]); ?></td>
						<td align="center"><div>
								<a href="#">作廢</a>|<a href="__URL__/InvoiceEdit?id=<?php echo ($item["id"]); ?>">修改</a> | <a onclick="return confirm('確認要刪除選擇的數據?')" href="__URL__/InvoiceDelete?id=<?php echo ($item["id"]); ?>">刪除</a>
							</div></td>
