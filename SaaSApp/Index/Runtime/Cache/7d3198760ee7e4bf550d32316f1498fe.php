...
<script type="text/javascript">
	$(function() {
		$("#shour").val('<?php echo ($business["shour"]); ?>');
		$("#sminute").val('<?php echo ($business["sminute"]); ?>');
		$("#ehour").val('<?php echo ($business["ehour"]); ?>');
		$("#eminute").val('<?php echo ($business["eminute"]); ?>');
		var spec='<?php echo ($business["specificmatters"]); ?>';
		var specs=document.getElementsByName("specific[]");
		for(var i=0;i<specs.length;i++)
			{
				if(spec.indexOf(specs[i].value)>-1)
					{
	...
	function check() {
		return validator.empty('date', '請選擇日期!') && validator.index('shour', '請選擇開始小時!') && validator.index('sminute', '請選擇開始分鐘!') && validator.index('ehour', '請選擇結束小時!') && validator.index('eminute', '請選擇結束分鐘!');
	}
</script>
</head>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<form action="__URL__/update" onsubmit="return check()" method="post" enctype="multipart/form-data" name="form1" id="form1">
					<input type="hidden" value="<?php echo ($business["id"]); ?>" name="id" />
					<div class="topic2">新增 / 修改紀錄</div>
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">

						<tr>
							<th width="135" scope="row">日期</th>
							<td width="557"><input type="text" name="date" id="date" value="<?php echo ($business["date"]); ?>" class="Wdate" onfocus="WdatePicker()" /></td>
						</tr>
						<tr>
							<th scope="row">時間</th>
							<td>開始 <select name="shour" id="shour">
						...
						<tr>
							...
							<td><input type="text" name="companyname" value="<?php echo ($business["companyname"]); ?>" style="width: 300px" /></td>
						</tr>
						<tr>
							<th scope="row">統一編號</th>
							<td><input type="text" name="businesscode" value="<?php echo ($business["businesscode"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">聯絡人</th>
							<td>姓名 <input type="text" name="linkername" value="<?php echo ($business["linkername"]); ?>" /> ；職位 <input type="text" name="linkerduty" value="<?php echo ($business["linkerduty"]); ?>" /> <br /> 電話 <input type="text" name="linkerphone" value="<?php echo ($business["linkerphone"]); ?>" /> ；手機 <input type="text" name="linkermobile" value="<?php echo ($business["linkermobile"]); ?>" /> <br /> Email <input type="text" name="linkeremail" value="<?php echo ($business["linkeremail"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">互動方式</th>
							<td><?php if($business["interactiveway"] == '' ): ?><input name="interactiveway" type="radio" value="電話聯繫" checked="checked" /> 電話聯繫 <input name="interactiveway" type="radio" value="親自拜訪" /> 親自拜訪<?php else: ?>
								<input name="interactiveway" type="radio" value="電話聯繫" /> 電話聯繫 <input name="interactiveway" checked="checked" type="radio" value="親自拜訪" /> 親自拜訪<?php endif; ?></td>
						</tr>
						<tr>
							<th scope="row">具體事項</th>
							<td><input type="checkbox" name="specific[]" value="初次拜訪" /> 初次拜訪 <input type="checkbox" name="specific[]" value="業務報價" /> 業務報價 <input type="checkbox" name="specific[]" value="工程報價" /> 工程報價 <input type="checkbox" name="specific[]" value="維修報價" /> 維修報價<br /> <input type="checkbox" name="specific[]" value="會議" /> 會議 <input type="checkbox" name="specific[]" value="簽約" /> 簽約 <input type="checkbox" name="specific[]" value="收款" /> 收款 <input type="checkbox" name="specific[]" value="維護" /> 維護</td>
						</tr>
						<tr>
							<th scope="row">狀態</th>
							<td><?php if(($business["state"])  ==  "預訂工作"): ?><input name="state" type="radio" value="預訂工作" checked="checked" /> 預訂工作 <input name="state" type="radio" value="完成工作" /> 完成工作<?php else: ?><input name="state" type="radio" value="預訂工作"  /> 預訂工作 <input name="state" type="radio" value="完成工作" checked="checked"/> 完成工作<?php endif; ?></td>
						</tr>
						<tr>
							<th scope="row">提醒</th>
							<td><?php if(($business["awoke"])  ==  "0"): ?><input name="awoke" type="radio" value="1" /> 加入提醒 <input name="awoke" type="radio" value="0" checked="checked" /> 不提醒<?php else: ?>
								<input name="awoke" type="radio" checked="checked" value="1" /> 加入提醒 <input name="awoke" type="radio" value="0"  /> 不提醒<?php endif; ?></td>
						</tr>
						<tr>
							<th scope="row">詳細說明</th>
							<td><label> <textarea name="detail" rows="6" style="width: 500px"><?php echo ($business["detail"]); ?></textarea>
							</label></td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
