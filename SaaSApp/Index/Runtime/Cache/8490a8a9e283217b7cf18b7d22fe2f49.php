...
<script type="text/javascript">
	$(function(){
		var leader="<?php echo ($depart["leader"]); ?>";
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<form action="__URL__/insert" onsubmit="return check();" method="post" enctype="multipart/form-data" name="form1" id="form1">
					...
					<?php if(($id)  !=  ""): ?><input type="hidden" value="<?php echo ($id); ?>" name="id"></input><?php endif; ?>
					<table width="695" border="0" align="center" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							...
							<td width="587"><input type="text" id="name" name="name" value="<?php echo ($depart["name"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">部門主管</th>
							<td><select name="leader" id="leader">
									<?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><option value="<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
							</select> </td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
				...
				<form id="form2" name="form2" method="post" action="__URL__/index">
					...
					<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
						...
						<td align="center"><?php echo ($item["leader"]); ?></td>
						<td align="center"><a href="__APP__/User/friend?depart=<?php echo ($item["id"]); ?>"><?php echo ($item["users"]); ?></a></td>
						<td><a href="__URL__/edit?id=<?php echo ($item["id"]); ?>">修改</a> | <a onclick="return confirm('確認要刪除數據嗎?');" href="__URL__/delete?id=<?php echo ($item["id"]); ?>">刪除</a> | <a href="#">部門資產</a></td>
