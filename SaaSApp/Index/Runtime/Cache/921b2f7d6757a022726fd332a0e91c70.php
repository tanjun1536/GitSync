			...
			<div class="mainInfo">
				...
				<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
					...
					<tr>
						...
						<td colspan="3"><?php echo ($meet["topic"]); ?></td>
					</tr>
					<tr>
						<th scope="row">開會時間</th>
						<td width="228"><?php echo ($meet["day"]); ?><?php echo ($meet["hours"]); ?>:<?php echo ($meet["minutes"]); ?></td>
						<th width="114" scope="row">地點</th>
						<td width="219"><?php echo ($meet["address"]); ?></td>
					</tr>
					<tr>
						<th scope="row">會議主持</th>
						<td><?php echo ($meet["hname"]); ?>(<?php echo ($meet["hucode"]); ?>)</td>
						<th scope="row">會議記錄人</th>
						<td><?php echo ($meet["rname"]); ?>(<?php echo ($meet["rucode"]); ?>)</td>
					</tr>
					<tr>
						<th scope="row">會議發起人</th>
						<td colspan="3"><label><?php echo ($meet["sname"]); ?>(<?php echo ($meet["sucode"]); ?>)</label></td>
					</tr>
					<tr>
						<th scope="row">與會人員</th>
						<td><ul>
								<?php $_result=explode(',',$meet['partersname']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): ++$i;$mod = ($i % 2 )?><li><?php echo ($user); ?></li><?php endforeach; endif; else: echo "" ;endif; ?>
							</ul></td>
						<th scope="row">實際出席人員</th>
						<td><ul>
								<?php $_result=explode(',',$meet['partersname']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): ++$i;$mod = ($i % 2 )?><li><label name="factpartersname[]"><?php echo ($user); ?></label></li><?php endforeach; endif; else: echo "" ;endif; ?>
							</ul></td>
					</tr>
					<tr>
						<th scope="row">相關附件</th>
						<td colspan="3"><?php if(($meet["atts"])  !=  ""): ?><input type="hidden" value="<?php echo ($meet["atts"]); ?>" name="atts"> </input> <?php $_result=explode(',',$meet['atts']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$att): ++$i;$mod = ($i % 2 )?><a href="__URL__/down?name=<?php echo (urlencode($att)); ?>" target="_blank"><?php echo ($att); ?></a>；<?php endforeach; endif; else: echo "" ;endif; ?><?php endif; ?></td>
					</tr>
					<tr>
						<th scope="row">會議內容描述</th>
						<td colspan="3"><?php echo ($meet["remark"]); ?></td>
					</tr>
				</table>
				<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
					<tr>
