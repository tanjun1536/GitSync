			...
			<div class="mainInfo">
				<div align="right" style="margin-right: 20px">
					<form id="form1" name="form1" method="post" action="">
						權限組： <select name="請選擇" onchange="MM_jumpMenu('parent',this,false)">
							<option value="__URL__/limit" selected="selected">所有同事</option>
							<?php if(is_array($roles)): $i = 0; $__LIST__ = $roles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$role): ++$i;$mod = ($i % 2 )?><?php if(!empty($R)): ?><?php if(($R)  ==  $role["id"]): ?><option selected="selected" value="__URL__/limit?role=<?php echo ($role["id"]); ?>"><?php echo ($role["name"]); ?></option>
							<?php else: ?>
							<option value="__URL__/limit?role=<?php echo ($role["id"]); ?>"><?php echo ($role["name"]); ?></option><?php endif; ?> <?php else: ?>
							<option value="__URL__/limit?role=<?php echo ($role["id"]); ?>"><?php echo ($role["name"]); ?></option><?php endif; ?><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</form>
				</div>
