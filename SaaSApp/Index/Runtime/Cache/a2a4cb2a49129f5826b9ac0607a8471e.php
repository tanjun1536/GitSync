...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="mailNav">
					<a href="__URL__/acceptAdd">新增案件</a>
				</div>
				<table width="695" border="0" cellpadding="0" cellspacing="0"
					class="tab_thtop">
					<tr>
						<th width="88" scope="col">日期時間</th>
						<th width="248" scope="col">案件名稱</th>
						<th width="215" scope="col">主要建物地址</th>
						<th width="100" scope="col">總金額</th>
						<th width="62" scope="col">操作</th>
					</tr>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($i%2 == 1): ?><tr>
						...
						<td align="center"><?php echo (formatDate($item["casedate"])); ?></td>
						<td align="center"><a href="__URL__/acceptCheckList?accept=<?php echo ($item["id"]); ?>"><?php echo ($item["casename"]); ?></a></td>
						<td align="center"><?php echo ($item["address"]); ?></td>
						<td align="center"><ul>
								<li><?php echo ($item["totalmoney"]); ?></li>
							</ul></td>
						<td align="center"><a href="__URL__/acceptEdit?id=<?php echo ($item["id"]); ?>">修改</a><a  onclick="return confirm('是否要刪除選擇的數據?')" href="__URL__/acceptDelete?id=<?php echo ($item["id"]); ?>">刪除</a></td>
