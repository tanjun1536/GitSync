...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">出勤紀錄 - 主管代打</div>
			<div class="mainInfo">
				<form id="form1" name="form1" method="post"
					action="__URL__/delegate">
					<div style="margin-bottom: 10px">
						<strong>組別： <select name="team">
								<option value="">所有組別</option>
								<?php if(is_array($teams)): $i = 0; $__LIST__ = $teams;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($item["id"] == $team): ?><option value="<?php echo ($item["id"]); ?>" selected="selected"><?php echo ($item["name"]); ?></option>
								<?php else: ?>
								<option value="<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></option><?php endif; ?><?php endforeach; endif; else: echo "" ;endif; ?>
						</select> 日期：
						</strong> <input type="text" name="date" style="width: 100px" class="Wdate"
							onfocus="WdatePicker()" value="<?php echo ($date); ?>" /> <input type="submit"
							name="Submit" value="搜尋" />
					</div>
					</form>
					<form id="form2" name="form1" method="post"
					action="__URL__/addDelegate">
					<input type="hidden" value="<?php echo ($date); ?>" name="date"/>
					<table width="695" border="0" cellpadding="0" cellspacing="0"
						class="tab_thtop">
						<tr>
							<th width="28" scope="col"><input type="checkbox"
								name="checkbox" value="checkbox" /></th>
							<th width="108" scope="col">日期</th>
							<th width="145" scope="col">類別</th>
							<th width="151" scope="col">員工</th>
							<th width="91" scope="col">上班時間</th>
							<th width="99" scope="col">下班時間</th>
							<th width="71" scope="col">時數</th>
						</tr>
						...
						<td align="center"><input type="checkbox"
							name="checkboxitems[]" value="<?php echo ($item["uid"]); ?>,<?php echo ($item["cid"]); ?>" /></td>
						<td align="center"><?php echo (date('Y/m/d',strtotime($item["datetime"]))); ?>
							(<?php echo (getWeek($item["datetime"])); ?>)</td>
						<td align="center"><?php echo ($item["type"]); ?></td>
						<td align="center"><?php echo ($item["name"]); ?>(<?php echo ($item["ucode"]); ?>)</td>
						<td align="center"><?php echo ($item["ontime"]); ?></td>
						<td align="center"><?php echo ($item["downtime"]); ?><?php if(!empty($item["overday"])): ?>+1<?php endif; ?></td>
						<td align="center"><?php echo ($item["difftime"]); ?></td>
						</tr><?php endforeach; endif; else: echo "" ;endif; ?>
					</table>
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th width="110" scope="row">主管代打</th>
							<td width="582"><select name="type">
									<option value="一般(主管代打)">一般</option>
									<option value="加班(主管代打)">加班</option>
							</select><br />上班 <select name="onhour">
							...
							</select> 時 <input type="text" name="onminute" style="width: 20PX" /> 分
								<br /> 下班 <select name="downhour">
									<option value="00">00</option>
									<option value="01">01</option>
									<option value="02">02</option>
									<option value="03">03</option>
									...
									<option value="22">22</option>
									<option value="23">23</option>
							</select> 時 <input type="text" name="downminute" style="width: 20PX" />
								分 <input type="checkbox" name="overday" value="1" /> 跨日</td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
							<td><input type="submit" name="Submit2" value="確定送出" /></td>
						</tr>
					</table>
				</form>
