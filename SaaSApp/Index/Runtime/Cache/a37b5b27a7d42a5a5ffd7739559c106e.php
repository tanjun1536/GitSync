...
var bt = "<?php echo ($cct["btype"]); ?>";
var mt = "<?php echo ($cct["mtype"]); ?>";
var ct = "<?php echo ($cct["ctype"]); ?>";
	$(function() {
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<form action="__URL__/addProductCChildType" onsubmit="return check();" method="post" enctype="multipart/form-data" name="form1" id="form1">
					<input type="hidden" name="id" style="width: 400px" value="<?php echo ($cct["id"]); ?>" />
...
<!-- 									<?php if(is_array($mtypes)): $i = 0; $__LIST__ = $mtypes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?>-->
<!-- 									<option value="<?php echo ($item["id"]); ?>"><?php echo ($item["name"]); ?></option> -->
<!--<?php endforeach; endif; else: echo "" ;endif; ?> -->
<!-- 							</select> <select name="ctype" id="ctype"><option value="">請選擇子類別</option> -->
<!-- 							</select> <input type="text" name="name" value="<?php echo ($cct["name"]); ?>" /> <input type="submit" name="Submit" value="確定" /></td> -->
<!-- 						</tr> -->
<!-- 					</table> -->
				</form>
				<form id="form2" name="form2" method="post" action="">
					...
					<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
						...
						<td align="center"><?php echo ($item["c1name"]); ?> &gt;<?php echo ($item["c2name"]); ?> &gt; <?php echo ($item["c3name"]); ?></td>
						<td align="center"><?php echo ($item["c4name"]); ?></td>
						<td align="center"><a href="__URL__/updateProductCChildType?id=<?php echo ($item["id"]); ?>">修改</a> | <a onclick="return confirm('是否刪除該數據?')" href="__URL__/deleteProductCChildType?id=<?php echo ($item["id"]); ?>">刪除</a></td>
