...
window.onload=function(){
	document.getElementById('stocktype').value='<?php echo ($type); ?>';
...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<form id="form1" name="form1" method="post" action="">
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							...
							<input name="sdate"  class="Wdate" onfocus="WdatePicker()" value="<?php echo ($sdate); ?>"/>至
							<input name="edate"  class="Wdate" onfocus="WdatePicker()" value="<?php echo ($edate); ?>"/>
				...
				<div class="mailNav">
				<?php if(check_power('新增調撥單',$ext) == 1): ?><a href="__URL__/stockAllocate">新增調撥單</a><?php endif; ?>
					<?php if(check_power('新增出庫單',$ext) == 1): ?><a href="__URL__/stockOut">新增出庫單</a><?php endif; ?>
					<?php if(check_power('出庫性質',$ext) == 1): ?><a href="__URL__/stockOutType">出庫性質</a><?php endif; ?>
					<?php if(check_power('新增入庫單',$ext) == 1): ?><a href="__URL__/stockIn">新增入庫單</a><?php endif; ?>
					<?php if(check_power('入庫性質',$ext) == 1): ?><a href="__URL__/stockInType">入庫性質</a><?php endif; ?>
				...
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					...
					<td align="center"><?php if($item["type"] == '出庫'): ?><a href="__URL__/stockOutView?id=<?php echo ($item["id"]); ?>"><?php echo ($item["outcode"]); ?></a> <?php elseif($item["type"] == '入庫' ): ?> <a href="__URL__/stockInView?id=<?php echo ($item["id"]); ?>"><?php echo ($item["incode"]); ?></a> <?php else: ?> <a href="__URL__/stockAllocateView?id=<?php echo ($item["id"]); ?>"><?php echo ($item["allocatecode"]); ?></a><?php endif; ?></td>
					<td><?php echo ($item["warehousecode"]); ?>/<?php echo ($item["warehousename"]); ?></td>
					...
					<td align="center"><?php if($item["state"] == '新進單'): ?><?php if($item["type"] == '出庫'): ?><a href="__URL__/stockOutCheckPage?id=<?php echo ($item["id"]); ?>">審核</a> 
					 <?php elseif($item["type"] == '入庫'): ?> 
					 <a href="__URL__/stockInCheckPage?id=<?php echo ($item["id"]); ?>">審核</a> <?php else: ?> 
					 <a href="__URL__/stockAllocateCheckPage?id=<?php echo ($item["id"]); ?>">審核</a><?php endif; ?>
					 <?php else: ?>
					 	<a href="#" onclick="MM_openBrWindow('__URL__/StockPrint?id=<?php echo ($item["id"]); ?>','','toolbar=yes,scrollbars=yes,width='+screen.width+',height='+screen.height)">列印</a><?php endif; ?></td>
