...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="topic2">新增/ 修改入庫性質</div>
				<form id="form1" name="form1" method="post"
					action="__URL__/addStockInType">
					<input type="hidden" name="id" value="<?php echo ($intype["id"]); ?>" /> <input
						type="hidden" name="code" style="width: 400px"
						value="<?php echo $_SESSION["SESSION_USER"]["code"];?>" />

					<table width="695" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th width="110" scope="row">類別</th>
							<td width="282"><select name="type" id="type">
									<option value="客戶">客戶</option>
									<option value="廠商">廠商</option>
									<option value="倉庫">倉庫</option>
							</select> </td>
							<th width="220" scope="row">性質名稱</th>
							<td width="282"> <input type="text" name="name" value="<?php echo ($intype["name"]); ?>" /> <input
								type="submit" name="Submit" value="確定" /></td>
						</tr>
					</table>
				</form>
				<table width="695" border="0" cellpadding="0" cellspacing="0"
					class="tab_thtop">
					<tr>
						<th width="87" scope="col">類別</th>
						<th width="527" scope="col">入庫性質</th>
						<th width="166" scope="col">操作</th>
					</tr>
					...
					<td align="center"><a
						href="__URL__/updateStockInType?id=<?php echo ($item["id"]); ?>">修改</a> | <a
						onclick="return confirm('是否刪除該數據?')"
						href="__URL__/deleteStockInType?id=<?php echo ($item["id"]); ?>">刪除</a></td>
