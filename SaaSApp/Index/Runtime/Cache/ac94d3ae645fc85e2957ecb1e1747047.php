...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="news_info">
				...
				<div class="txt_lightblue" style="margin-bottom: 20px"><?php echo ($news["senddate"]); ?> <?php echo ($news["uname"]); ?> 發佈</div>
				<div><?php echo ($news["content"]); ?></div>
				<div class="words01">
					附件：<?php if(!empty($news["atts"])): ?><?php $_result=explode(',',$news['atts']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$att): ++$i;$mod = ($i % 2 )?><a href="__URL__/down?name=<?php echo ($att); ?>">
						   <img style="border: 0" src="__PUBLIC__/images/063.gif"></img><?php echo ($att); ?></a><?php endforeach; endif; else: echo "" ;endif; ?><?php endif; ?> <br />
					</a>相關連結：<a href="<?php echo ($news["link"]); ?>" target="_blank"><?php echo ($news["link"]); ?></a>
				</div>
				<div class="back">
					<a href="__URL__/index">》 回上頁 </a>
				</div>
			</div>
		</div>
	</div>
