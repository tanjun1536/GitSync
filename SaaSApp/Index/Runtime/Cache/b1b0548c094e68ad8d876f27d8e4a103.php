		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<form id="form1" name="form1" method="post" action="__URL__/addBid">
					<input type="hidden" id="biddate" name="biddate" /> <input type="hidden" name="id" value="<?php echo ($bid["id"]); ?>" /> <input type="hidden" name="code" value="<?php echo $_SESSION["SESSION_USER"]["code"];?>" /> <input type="hidden" name="state" value="報價" />
					<div class="topic2">新增／修改報價單</div>
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="116" scope="row">報價單編號</th>
							<td width="228"><input type="text" name="bidcode" value="<?php echo ($bid["bidcode"]); ?>" /></td>
							<th width="100" scope="row">報價日期</th>
							<td width="246" id="tdDate">系統抓當日</td>
						</tr>
						<tr>
							<th scope="row">客戶名稱</th>
							<td><input type="text" name="clientname" value="<?php echo ($bid["clientname"]); ?>" /></td>
							<th scope="row">統一編號</th>
							<td><input type="text" name="uniquecode" value="<?php echo ($bid["uniquecode"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">聯絡人</th>
							<td><input type="text" name="linker" value="<?php echo ($bid["linker"]); ?>" /></td>
							<th scope="row">聯絡電話</th>
							<td><input type="text" name="linkerphone" value="<?php echo ($bid["linkerphone"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">報價金額</th>
							<td><input type="text" name="bidprice" value="<?php echo ($bid["bidprice"]); ?>" /></td>
							<th scope="row">發票種類</th>
							<td><?php if(empty($bid["billtype"])): ?><input name="billtype" type="radio" checked="checked" value="無" /> 無 <input name="billtype" type="radio" value="二聯" /> 二聯 <input name="billtype" type="radio" value="三聯" /> 三聯<?php else: ?> <?php if($bid["billtype"] == '无'): ?><input name="billtype" type="radio" checked="checked" value="無" /> 無 <input name="billtype" type="radio" value="二聯" /> 二聯 <input name="billtype" type="radio" value="三聯" /> 三聯 <?php elseif($bid["billtype"] == '二聯'): ?> <input name="billtype" type="radio" value="無" /> 無 <input name="billtype" type="radio" value="二聯" checked="checked" /> 二聯 <input name="billtype" type="radio" value="三聯" /> 三聯 <?php else: ?> <input name="billtype"
									type="radio" value="無" /> 無 <input name="billtype" type="radio" value="二聯" /> 二聯 <input name="billtype" checked="checked" type="radio" value="三聯" /> 三聯<?php endif; ?><?php endif; ?></td>
						</tr>
						<tr>
							<th scope="row">簽約金額</th>
							<td><input type="text" name="signprice" value="<?php echo ($bid["signprice"]); ?>" /></td>
							<th scope="row">發票號碼</th>
							<td><input type="text" name="billcode" value="<?php echo ($bid["billcode"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">成交金額</th>
							<td colspan="3"><input type="text" name="concludedprice" value="<?php echo ($bid["concludedprice"]); ?>" /></td>
						</tr>
						<tr>
							...
							<td colspan="3"><table width="550" border="0" cellspacing="0" cellpadding="0" id="biddetail">
									...
									<?php if(is_array($detail)): $i = 0; $__LIST__ = $detail;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><tr><input type="hidden" value="<?php echo ($item["name"]); ?>,<?php echo ($item["unit"]); ?>,<?php echo ($item["price"]); ?>" name ='detail[]' />
						...
						<tr>
							<th scope="row">備註</th>
							<td colspan="3"><textarea name="remark" rows="5" style="width: 500px"><?php echo ($bid["remark"]); ?></textarea></td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
