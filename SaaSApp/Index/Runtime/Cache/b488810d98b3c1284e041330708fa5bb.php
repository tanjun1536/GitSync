...
<!-- 				</ul></li> -->
  ...
  <?php echo $_SESSION["caseName"];?>
			...
			<div class="topic">專案管理 - 專案內容</div>
			<div class="mainInfo">
				<div class="mailNav">
					<?php if(check_power('修改專案',$ext) == 1): ?><a href="__URL__/edit?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($c["id"]); ?>">修改專案</a><?php endif; ?>
				</div>
				<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">

					<tr>
						<th scope="row">專案名稱</th>
						<td colspan="3"><?php echo ($c["name"]); ?></td>
					</tr>
					<tr>
						<th scope="row">客戶名稱</th>
						<td colspan="3"><?php echo ($c["clientname"]); ?></td>
					</tr>
					<tr>
						<th width="119" scope="row">合約編號</th>
						<td width="213"><?php echo ($c["contractcode"]); ?></td>
						<th width="137" scope="row">專案編號</th>
						<td width="221"><?php echo ($c["casecode"]); ?></td>
					</tr>
					<tr>
						<th scope="row">合約起始日</th>
						<td><?php echo (formatDate($c["contractstartdate"])); ?></td>
						<th width="137" scope="row">專案起始日</th>
						<td><?php echo (formatDate($c["casestartdate"])); ?></td>
					</tr>
					<tr>
						<th scope="row">合約結束日</th>
						<td><?php echo (formatDate($c["contractenddate"])); ?></td>
						<th width="137" scope="row">專案預定結束日</th>
						<td><?php echo (formatDate($c["caseevaluateenddate"])); ?></td>
					</tr>
					<tr>
						<th scope="row">合約金額</th>
						<td><?php echo ($c["contractmoney"]); ?></td>
						<th scope="row">專案實際結束日</th>
						<td><?php echo (formatDate($c["casereallyenddate"])); ?></td>
					</tr>
					<tr>
						<th scope="row">專案負責人</th>
						<td colspan="3"><?php echo ($c["zaname"]); ?>(<?php echo ($c["zacode"]); ?>)</td>
					</tr>
					<tr>
						<th scope="row">業務負責人</th>
						<td colspan="3"><?php echo ($c["ywname"]); ?>(<?php echo ($c["ywcode"]); ?>)</td>
					</tr>
					<tr>
						<th scope="row">專案督導</th>
						<td colspan="3"><?php echo ($c["ddname"]); ?>(<?php echo ($c["ddcode"]); ?>)</td>
					</tr>
					<tr>
						<th scope="row">專案所屬人員</th>
						<td colspan="3"><label><?php echo (str_replace(',','；',$c["casebelongusersname"])); ?></label></td>
					</tr>
					<tr>
						<th scope="row">專案查看人員</th>
						<td colspan="3"><?php echo (str_replace(',','；',$c["caselookusersname"])); ?></td>
					</tr>
					<tr>
						<th scope="row">專案內容描述</th>
						<td colspan="3"><?php echo ($c["casecontentdesc"]); ?></td>
					</tr>
				</table>
			</div>
		</div>
