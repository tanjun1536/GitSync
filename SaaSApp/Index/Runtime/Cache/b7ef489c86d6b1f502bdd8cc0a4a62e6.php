...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
			...
			<div class="mainInfo">
				<form action="__URL__/acceptSave" method="post"
					...
					<div class="topic2">新增/修改案件</div>
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th width="111" scope="row">日期</th>
							<td width="581"><input type="text" name="casedate" class="Wdate" onfocus="WdatePicker()" value="<?php echo ($accept["casedate"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">案件名稱</th>
							<td><input type="text" name="casename" value="<?php echo ($accept["casename"]); ?>"
								style="width: 400px" /></td>
						</tr>
						<tr>
							<th scope="row">主要建物地址</th>
							<td><input type="text" name="address" value="<?php echo ($accept["address"]); ?>"
								style="width: 400px" /></td>
						</tr>
						<tr>
							<th scope="row">總金額</th>
							<td><input type="text" name="totalmoney" value="<?php echo ($accept["totalmoney"]); ?>"/></td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
