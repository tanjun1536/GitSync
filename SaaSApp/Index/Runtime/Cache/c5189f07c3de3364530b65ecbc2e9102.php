...
<script type="text/JavaScript">
	$(function() {
		...
		$("#inhour").val(new Date().getHours());
		$("#inminute").val(new Date().getMinutes());
	...
	function check() {
		...
		if (validator.empty('inwarehousecode', '請輸入倉庫代號')
				&& validator.index('intype', '請選擇入庫性質')) {
			if ($("#tdinwarehousename").html() == '') {
				alert('請填寫倉庫名稱!');
				return false;
			}
			if ($("#totalmoney").val() == '0') {
...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="topic2">新增入庫單</div>
				<form id="form1" name="form1" method="post"
					onsubmit="return check()" action="__URL__/addStockIn">
					<input type="hidden" name="totalmoney" id="totalmoney" value="0" />
					<input type="hidden" name="code" value="<?php echo $_SESSION["SESSION_USER"]["code"];?>" /> <input
						type="hidden" name="inuser" value="<?php echo $_SESSION["SESSION_USER"]["name"];?>" /> <input
						type="hidden" name="indate" id="indate" /> <input type="hidden"
						name="inwarehousename" id="inwarehousename" /> <input
						type="hidden" name="type" value="入庫" /> <input type="hidden"
						name="state" value="新進單" /> <input type="hidden"
						name="intypename" id="intypename" />
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th width="91" scope="row">倉庫代號</th>
							<td width="142"><input type="text" name="inwarehousecode"
								id="inwarehousecode" /> <input type="button" name="Submit3"
								value="確定" onclick="getStock()" /></td>
							<th width="91" scope="row">倉庫名稱</th>
							<td width="266" id="tdinwarehousename"></td>
						</tr>
						<tr>
							<th scope="row">入庫性質</th>
							<td><select name="intype" id="intype"
								onchange="$('#intypename').val(this.options[this.selectedIndex].text)">
							...
							</select> 時 <input type="text" name="inminute" id="inminute"
						...
						<tr>
							...
							</select> <input type="text" name="objname" id="objname"
								value="<?php echo ($orderuser); ?>" /> <input type="button"
								onclick="getObjs()" value="查詢" /> <select name="objid"
								id="objid">
									<option value="查詢結果">查詢結果</option>
							</select><br></br></td>
						</tr>
						<tr>
							<th scope="row">入庫單號</th>
							<td colspan="3"><input type="text" name="incode"
								value="<?php echo ($incode); ?>"></td>
						</tr>
						<tr>
					...
					<table width="695" border="0" cellpadding="0" cellspacing="0"
						...
						<tr>
							<td align="center"><input type="hidden" id="productunit" />
								<input type="hidden" id="productmodel" /> <input type="hidden"
								id="productsize" /> <input type="hidden" id="productcp" /> <input
								type="hidden" id="productcd" /><input type="text"
								name="productcode" id="productcode" style="width: 80px" /><input
								type="button" value="搜尋" onclick="getProduct()" /></td>
							<td id="productname">&nbsp;</td>
							<td align="center"><input type="text" name="productprice"
								id="productprice" style="width: 80px" /></td>
							<td align="center"><input type="text" id="quantity"
								style="width: 50px" /></td>
							<td align="center" id="subtotal">&nbsp;</td>
							<td><input type="text" name="productremark"
								id="productremark" style="width: 140px" /></td>
						...
						<tr class="trcolor3">
							...
							<td colspan="4" bgcolor="#FFFFEC"><input type="radio"
								name="taketoaccount" checked="checked" value="1" /> 拋轉會計系統<input
								type="radio" name="taketoaccount" value="0" /> 不必拋轉會計系統</td>
