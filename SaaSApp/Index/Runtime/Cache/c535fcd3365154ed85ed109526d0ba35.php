...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="topic2">新增/ 修改出庫性質</div>
				<form id="form1" name="form1" method="post"
					action="__URL__/addStockOutType">
					<input type="hidden" name="id" value="<?php echo ($outtype["id"]); ?>" /> <input
					...
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th width="110" scope="row">性質名稱</th>
							<td width="582"><input type="text" name="name"
								value="<?php echo ($outtype["name"]); ?>" /> <input type="submit" name="Submit"
								value="確定" /></td>
						</tr>
						<tr>
							<th scope="row">類型</th>
				...
				<table width="695" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<th width="527" scope="col">出庫性質</th>
					...
					<td align="center"><a
						href="__URL__/updateStockOutType?id=<?php echo ($item["id"]); ?>">修改</a> | <a
						onclick="return confirm('是否刪除該數據?')"
						href="__URL__/deleteStockOutType?id=<?php echo ($item["id"]); ?>">刪除</a></td>
