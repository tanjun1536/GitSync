		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<form id="form2" name="form2" method="post" action="__URL__/index">
					<div class="mailNav">
						<?php if(check_power('新增訊息',$ext) == 1): ?><a href="__URL__/add">新增訊息</a><?php endif; ?>
				...
				<form id="form1" name="form1" method="post" onsubmit="return delAwoke();" action="__URL__/delete">
					<table width="695" border="0" cellpadding="0" cellspacing="0" class="news_list">
						...
						<td align="center"><?php echo (date('Y-m-d',strtotime($item["senddate"]))); ?></td>
						<td><?php if(!empty($item["atts"])): ?><?php $_result=explode(',',$item['atts']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$att): ++$i;$mod = ($i % 2 )?><a href="__URL__/down?name=<?php echo ($att); ?>">
						   <img style="border: 0" src="__PUBLIC__/images/063.gif"></img></a><?php endforeach; endif; else: echo "" ;endif; ?><?php endif; ?> 
						   <a href="__URL__/view?id=<?php echo ($item["id"]); ?>"><?php echo ($item["title"]); ?></a></td>
						<td align="center"><?php echo ($item["uname"]); ?></td>
						<td align="center"><?php echo ($item["readercount"]); ?> (<a href="#" onclick="MM_openBrWindow('__URL__/viewreaders?id=<?php echo ($item["id"]); ?>&depart=','','width=775,height=500')">明細</a>)
						</td>
						</tr><?php endforeach; endif; else: echo "" ;endif; ?>
					</table>
					<div style="margin-top: 10px">
						<?php if(check_power('刪除訊息',$ext) == 1): ?><input type="submit" name="Submit" value="刪除" /><?php endif; ?>
					</div>
				</form>
