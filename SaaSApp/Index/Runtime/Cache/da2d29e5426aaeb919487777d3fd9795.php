...
<!-- 				</ul></li> -->
  ...
  <?php echo $_SESSION["caseName"];?>
			...
			<div class="mainInfo">
				<form action="__URL__/attendanceImport?case=<?php echo $_SESSION["case"];?>" onsubmit="return check()" method="post" enctype="multipart/form-data" name="form1" id="form1">
					<input type="hidden" name="case" value="<?php echo $_SESSION["case"];?>" /> <input type="hidden" id="year" name="year" /> <input type="hidden" id="state" name="state" value="提交審核" />
				...
				<form id="form2" name="form2" method="post" action="__URL__/attendance?case=<?php echo $_SESSION["case"];?>">
					<div class="mailNav">
				...
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					...
					<td align="center"><?php echo ($item["updatedate"]); ?></td>
					<td align="center"><a href="__URL__/attendanceAudit?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>">審核 </a>| 
					<a href="__URL__/attendanceCheck?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>" target="_blank">檢視 </a>|
					<a href="__URL__/attendanceExport?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>&year=<?php echo ($item["year"]); ?>&month=<?php echo ($item["month"]); ?>" target="_blank">匯出 </a>|
					<a onclick="return confirm('是否刪除選擇的數據?')" href="__URL__/deleteAttendance?case=<?php echo $_SESSION["case"];?>&id=<?php echo ($item["id"]); ?>"> 刪除</a></td>
