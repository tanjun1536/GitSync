...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="topic2">
					<a href="__URL__/stockView?id=<?php echo ($id); ?>"><?php echo ($name); ?></a> - 新增/修改檢驗管理
				</div>
				<form id="form2" name="form2" method="post" action="__URL__/StockTestSave?id=<?php echo ($id); ?>">
					<input type="hidden" name="id" value="<?php echo ($test["id"]); ?>" /> 
					<input type="hidden" name="productname" id="productname" value="<?php echo ($test["productname"]); ?>" />
					 <input type="hidden" name="lastchecktime" value="<?php echo ($test["lastchecktime"]); ?>" /> 
					 <input type="hidden" name="state" value="需檢驗" />
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="130" scope="row">商品編號</th>
							<td width="562"><input type="text" name="productcode" id="productcode" value="<?php echo ($test["productcode"]); ?>" /></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><?php echo ($test["lastchecktime"]); ?></td>
						</tr>
						<tr>
						...
						<tr>
							<th scope="row">備註</th>
							<td><textarea name="remark" rows="6" style="width: 500px"><?php echo ($test["remark"]); ?></textarea></td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
