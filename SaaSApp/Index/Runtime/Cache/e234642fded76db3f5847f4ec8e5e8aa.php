...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="mainInfo">
				<div class="topic2">
					<a href="__URL__/stockView?id=<?php echo ($id); ?>"><?php echo ($name); ?></a> - 檢驗管理
				</div>
				<form id="form2" name="form2" method="post" action="__URL__/StockTestDetlete?id=<?php echo ($id); ?>">
					<div class="mailNav">
						<a href="__URL__/StockTestAdd?id=<?php echo ($id); ?>">新增檢驗管理</a>
					</div>
					<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
						<tr>
							<th width="28" scope="col"><input type="checkbox" name="checkbox" id="allCheck" onclick="CheckAll()"/></th>
						...
						<td align="center"><?php echo ($item["lastchecktime"]); ?></td>
						<td align="center"><?php echo ($item["nextchecktime"]); ?></td>
						<td align="center"><?php echo ($item["state"]); ?></td>
						<td align="center"><div>
								<a href="__URL__/StockTestEdit?id=<?php echo ($id); ?>&testid=<?php echo ($item["id"]); ?>">修改</a>
								<?php if($item["state"] == '需檢驗'): ?><a href="__URL__/StockTestCheck?id=<?php echo ($id); ?>&testid=<?php echo ($item["id"]); ?>">檢驗</a><?php endif; ?>
							</div></td>
