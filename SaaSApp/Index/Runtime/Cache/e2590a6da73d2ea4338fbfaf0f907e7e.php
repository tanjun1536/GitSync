...
<script type="text/javascript">
	...
	$(function(){
		$("input[type='radio'][value='<?php echo ($acc["accepttype"]); ?>']").attr("checked",true);
	});
	function addFile() {
...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
			...
			<div class="mainInfo">
				<form action="__URL__/acceptCheckSave?accept=<?php echo $_GET["accept"];?>" onsubmit="return check()"
					...
					<input type="hidden" name="id" value="<?php echo $_GET["id"];?>" /> <input
						type="hidden" name="acceptusers" id="acceptusers" value="<?php echo ($acc["acceptusers"]); ?>"/>
					<div class="topic2">新增點交紀錄</div>
					<table width="695" border="0" cellpadding="0" cellspacing="1"
						class="tabThSide">
						<tr>
							<th width="111" scope="row">日期</th>
							<td width="581"><input type="text" name="acceptdate" value="<?php echo ($acc["acceptdate"]); ?>"
								class="Wdate" onfocus="WdatePicker()" /></td>
						</tr>
						<tr>
							<th scope="row">工程名稱</th>
							<td><input type="text" name="acceptproject" value="<?php echo ($acc["acceptproject"]); ?>"
								style="width: 400px" /></td>
						</tr>
						<tr>
							<th scope="row">地點</th>
							<td><input type="text" name="acceptaddress" value="<?php echo ($acc["acceptaddress"]); ?>"
								style="width: 400px" /></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><textarea name="acceptcontent" rows="5"
									style="width: 500px"><?php echo ($acc["acceptcontent"]); ?></textarea></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><input type="text" name="acceptuser" id="acceptuser" />
								...
								<div>
									<ul id="users">
										<?php $_result=explode(',',$acc['acceptusers']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><li><span><?php echo ($item); ?></span><a href="javascript:void(0)"
