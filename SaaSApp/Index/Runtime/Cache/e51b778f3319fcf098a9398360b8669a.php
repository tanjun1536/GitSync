...
<!--
	...
	function check() {
		return validator.empty('lcode', '請輸入級別代號!') && validator.empty('remark', '請輸入備註!');
	}
//-->
</script>
</head>
...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">級別管理</div>
			<div class="mainInfo">
				<form onsubmit="return check()" action="__URL__/insert" method="post" enctype="multipart/form-data" name="form1" id="form1">
					<div class="topic2">新增 / 修改級別</div>
					<?php if(($level)  !=  ""): ?><input type="hidden"  name="id" value="<?php echo ($level["id"]); ?>"><?php endif; ?>
					<table width="695" border="0" align="center" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th width="93" scope="row">級別代號</th>
							<td width="197"><input type="text" id="lcode" value="<?php echo ($level["lcode"]); ?>" name="lcode" style="width: 100px" /></td>
							<th width="92" scope="row">備註</th>
							<td width="308"><input type="text" id="remark" value="<?php echo ($level["remark"]); ?>"  name="remark" style="width: 250px" /></td>
						</tr>
						<tr>
							<th width="93" scope="row">排序</th>
							<td width="197"><input type="text" id="ordernumber" value="<?php echo ($level["ordernumber"]); ?>" name="lcode" style="width: 100px" /></td>
							<th width="92" scope="row"></th>
							<td width="308"></td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
							<td colspan="3"><input type="submit" name="Submit" value="確定" /> <input type="reset" name="Submit2" value="復原" /></td>
						</tr>
					</table>
				</form>
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					<tr>
						<th width="158" scope="col">級別代號</th>
						<th width="361" scope="col">備註</th>
						<th width="64" scope="col">人數</th>
						<th width="110" scope="col">操作</th>
					</tr>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($i%2 == 1): ?><tr>
						...
						<td align="center"><?php echo ($item["lcode"]); ?></td>
						<td><?php echo ($item["remark"]); ?></td>
						<td align="center"><a href="#" onclick="MM_openBrWindow('__APP__/User/friendforlevel?level=<?php echo ($item["id"]); ?>','','width=775,height=500')"><?php echo ($item["users"]); ?></a></td>
						<td align="center"><a href="__URL__/edit?id=<?php echo ($item["id"]); ?>">修改</a> | <a onclick="return confirm('確認刪除數據?')" href="__URL__/delete?id=<?php echo ($item["id"]); ?>">刪除</a></td>
