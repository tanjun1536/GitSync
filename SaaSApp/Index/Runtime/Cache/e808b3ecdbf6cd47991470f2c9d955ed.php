...
<script type="text/JavaScript">
	//借方
	var jfsmt = "<?php echo ($in["jfsubjectmaintype"]); ?>";
	var jfsct = "<?php echo ($in["jfsubjectchildtype"]); ?>";
	var jfs = "<?php echo ($in["jfsubject"]); ?>";
	//贷方
	var dfsmt = "<?php echo ($in["dfsubjectmaintype"]); ?>";
	var dfsct = "<?php echo ($in["dfsubjectchildtype"]); ?>";
	var dfs = "<?php echo ($in["dfsubject"]); ?>";
	//兑现借方
	var dxjfsmt = "<?php echo ($in["dxjfsubjectmaintype"]); ?>";
	var dxjfsct = "<?php echo ($in["dxjfsubjectchildtype"]); ?>";
	var dxjfs = "<?php echo ($in["dxjfsubject"]); ?>";
	//兑现贷方
	var dxdfsmt = "<?php echo ($in["dxdfsubjectmaintype"]); ?>";
	var dxdfsct = "<?php echo ($in["dxdfsubjectchildtype"]); ?>";
	var dxdfs = "<?php echo ($in["dxdfsubject"]); ?>";
	//对象
	var ot = "<?php echo ($in["objecttype"]); ?>";
	var oi = "<?php echo ($in["objectid"]); ?>";
	var on = "<?php echo ($in["objectname"]); ?>";
	//账户
	var a = "<?php echo ($in["account"]); ?>";
	var bn="<?php echo ($in["bankname"]); ?>";
	var u="<?php echo ($in["username"]); ?>";
	//付款方式和状态
	var pm = "<?php echo ($in["paymode"]); ?>";
	var ps = "<?php echo ($in["paystate"]); ?>";

	var ddd2 = "<?php echo ($in["dddate2"]); ?>";
	var ddp2 = "<?php echo ($in["ddperson2"]); ?>";
	var depart = "<?php echo ($in["depart"]); ?>";
	$(function() {
		...
		if (!ddp2) {
			$("#ddperson2").val("<?php echo $_SESSION["SESSION_USER"]["name"];?>");
			$("#tdddperson2").html("<?php echo $_SESSION["SESSION_USER"]["name"];?>");
		}
...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
			...
			<div class="mainInfo">
				<form action="__URL__/addIncome" method="post" enctype="multipart/form-data" name="form1" onsubmit="return check()" id="form1">
					<input type="hidden" name="code" value="<?php echo $_SESSION["SESSION_USER"]["code"];?>" /> <input type="hidden" name="id" value="<?php echo ($in["id"]); ?>" />
					<div class="topic2">新增紀錄</div>
					<table width="695" border="0" cellpadding="0" cellspacing="1" class="tabThSide">
						<tr>
							<th scope="row">日期</th>
							<td><input type="text" name="indate" value="<?php echo (date('Y/m/d',strtotime($in["indate"]))); ?>" class="Wdate" onfocus="WdatePicker()" /></td>
						</tr>
						<tr>
							<th width="131" scope="row">所屬部門</th>
							<td width="561"><input type="hidden" name="departname" id="departname" value="<?php echo ($in["departname"]); ?>" />
						...
						<tr>
							...
							<td><?php if(($in["estimateclearancedate"])  !=  "0000-00-00 00:00:00"): ?><input type="text" name="estimateclearancedate" value="<?php echo (date('Y/m/d',strtotime($in["estimateclearancedate"]))); ?>" class="Wdate" onfocus="WdatePicker()" /> <?php else: ?> <input type="text" name="estimateclearancedate" class="Wdate" onfocus="WdatePicker()" /><?php endif; ?></td>
						</tr>
						<tr>
							<th scope="row">實際兌現日期</th>
							<td><?php if(($in["actualclearancedate"])  !=  "0000-00-00 00:00:00"): ?><input type="text" name="actualclearancedate" value="<?php echo (date('Y/m/d',strtotime($in["actualclearancedate"]))); ?>" class="Wdate" onfocus="WdatePicker()" /> <?php else: ?> <input type="text" name="actualclearancedate" class="Wdate" onfocus="WdatePicker()" /><?php endif; ?></td>
						</tr>
						<tr>
							<th scope="row">退票日期</th>
							<td><?php if(($in["backbilldate"])  !=  "0000-00-00 00:00:00"): ?><input type="text" name="backbilldate" value="<?php echo (date('Y/m/d',strtotime($in["backbilldate"]))); ?>" class="Wdate" onfocus="WdatePicker()" /> <?php else: ?> <input type="text" name="backbilldate" class="Wdate" onfocus="WdatePicker()" /><?php endif; ?></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><input type="text" name="billcode" value="<?php echo ($in["billcode"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">支票抬頭</th>
							<td><input type="text" name="billtitle" value="<?php echo ($in["billtitle"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">支票金額</th>
							<td><input type="text" name="billmoney" value="<?php echo ($in["billmoney"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">支票到期日</th>
							<td><?php if(($in["billexpiredate"])  !=  "0000-00-00 00:00:00"): ?><input type="text" name="billexpiredate" value="<?php echo (date('Y/m/d',strtotime($in["billexpiredate"]))); ?>" class="Wdate" onfocus="WdatePicker()" /> <?php else: ?> <input type="text" name="billexpiredate" class="Wdate" onfocus="WdatePicker()" /><?php endif; ?></td>
						</tr>
						<tr>
							<th scope="row">支付銀行帳號</th>
							<td><input type="text" name="paybankaccount" value="<?php echo ($in["paybankaccount"]); ?>" /></td>
						</tr>
						<tr>
							<th colspan="2" scope="row"><div align="center">
						...
						<tr>
							...
							<td id="tdbankname"><?php echo ($in["bankname"]); ?></td>
						</tr>
						<tr>
							<input type="hidden" id="username" name="username" />
							<th scope="row">戶名</th>
							<td id="tdusername"><?php echo ($in["username"]); ?></td>
						</tr>
						<tr>
							<th scope="row">金額</th>
							<td><input type="text" name="incomemoney" value="<?php echo ($in["incomemoney"]); ?>" /></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td id="tddddate1"><?php if(($in["dddate1"])  !=  "0000-00-00 00:00:00"): ?><?php echo (date('Y/m/d',strtotime($in["ddate1"]))); ?><?php endif; ?></td>
						</tr>
						<tr>
							<input type="hidden" id="ddperson1" name="ddperson1" value="<?php echo $_SESSION["SESSION_USER"]["name"];?>" />
							<th scope="row">登打人</th>
							<td id="tdddperson1"><?php echo ($in["ddperson1"]); ?></td>
						</tr>
						<tr>
							<th scope="row">實收金額</th>
							<td><input type="text" name="actualreceivemoney" value="<?php echo ($in["actualreceivemoney"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">差額科目</th>
							<td><input type="text" name="cesubject" value="<?php echo ($in["cesubject"]); ?>" /></td>
						</tr>
						<tr>
							<th scope="row">差額</th>
							<td><input type="text" name="ce" value="<?php echo ($in["ce"]); ?>" /></td>
						</tr>
						<tr>
							<input type="hidden" id="dddate2" name="dddate2" />
							<th scope="row">登打時間</th>
							<td id="tddddate2"><?php if(($in["dddate2"])  !=  "0000-00-00 00:00:00"): ?><?php echo (date('Y/m/d',strtotime($in["ddate2"]))); ?><?php endif; ?></td>
						</tr>
						<tr>
							<input type="hidden" id="ddperson2" name="ddperson2" />
							<th scope="row">登打人</th>
							<td id="tdddperson2"><?php echo ($in["ddperson2"]); ?></td>
						</tr>
						<tr>
							<th scope="row">備註</th>
							<td><textarea name=remark rows="6" style="width: 400px"><?php echo ($in["remark"]); ?></textarea></td>
						</tr>
						<tr>
							<th scope="row">&nbsp;</th>
