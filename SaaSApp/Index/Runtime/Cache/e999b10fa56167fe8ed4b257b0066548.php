			...
			<div class="mainInfo">
				<div class="mailNav">
					<form id="form2" name="form2" method="post" action="__URL__/index">
						<a href="__URL__/add">新增獎懲</a> <input type="text" name="key" /> <input type="submit" name="Submit3" value="搜尋" />
					</form>
				</div>
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					<tr>
						<th width="83" scope="col">獎懲時間</th>
						<th width="110" scope="col">員工姓名</th>
						<th width="140" scope="col">獎懲事由</th>
						<th width="191" scope="col">獎懲內容</th>
						<th width="118" scope="col">附件</th>
					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if($i%2 == 1): ?><tr><?php else: ?><tr class="trcolor"><?php endif; ?> 
						...
						<td align="center"><img width="100px" height="100px" src="__PUBLIC__/Uploads/thumb_<?php echo ($item["attachement"]); ?>" />
						</td>
						<td align="center"><a href="__URL__/edit?id=<?php echo ($item["id"]); ?>">修改</a></td>
