...
<?php if(is_array($TopMenu)): $i = 0; $__LIST__ = $TopMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): ++$i;$mod = ($i % 2 )?><?php if(empty($item["children"])): ?><li><a href="<?php echo ($item["link"]); ?>"><?php echo ($item["name"]); ?></a></li>
		...
		<li><a href="__APP__/Favorite">我的最愛</a>  </li>
			...
			<div class="topic">出勤紀錄 - 出勤管理</div>
			<div class="mainInfo">
				<div style="margin-bottom: 10px">
					<form id="form1" name="form1" method="post" action="__URL__/viewperson">
						<input type="hidden" name="pid" value="<?php echo ($pid); ?>"/>
						日期 <input type="text" name="sdate" style="width: 100px" class="Wdate" onfocus="WdatePicker()" value="<?php echo ($sdate); ?>" /> ~ <input type="text" name="edate" style="width: 100px" class="Wdate" onfocus="WdatePicker()" value="<?php echo ($edate); ?>"/> <input type="submit" name="Submit" value="搜尋" />
					</form>
				</div>
				<table width="695" border="0" cellpadding="0" cellspacing="0" class="tab_thtop">
					...
					<td align="center"><a href="check_inPersonnel.php"><?php echo ($item["name"]); ?>(<?php echo ($item["ucode"]); ?>)</a></td>
